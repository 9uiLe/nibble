...
現時点では開発ルールと設計に必要な知識を整備する段階です。アプリ本体、Xcode プロジェクト、技術スタックの最終決定はまだありません。
...
初回は lock に固定された依存を取得します。開発シェルでは Python 3・PyYAML・actionlint・ShellCheck が利用できます。シェルを終了するときは `exit` を実行してください。
...
このコマンドは開発シェルの内外から実行できます。`workflow-policy` と `nix-format` のチェックが成功すれば、補助ツールのセットアップは完了です。runner 方針、workflow の構文・シェル、Nix 定義の書式を検査します。
...
現在はアプリ本体と Xcode プロジェクトがないため、ここまでで開発準備は完了です。アプリのビルド・起動・テスト手順は、プロジェクト作成時に実際の scheme とともに追記します。
