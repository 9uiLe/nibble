...
本文に`{{宛名}}`のような変数を入れると、コピーやキーボード挿入の直前に値を指定できます。編集画面の「変数を追加」では、入力済みの変数名を再利用するか、新しい名前を作るかを選びます。同じ名前を本文中に複数回使っても入力欄は一つです。差し替えは利用する文章だけに適用し、保存済みの本文は変えません。値を確定する前に戻ると、コピー・挿入・使用記録は行いません。
