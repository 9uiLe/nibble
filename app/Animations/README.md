...
「nibbleについて」は文章のコピーと保存、キーボード利用案内は保存済み本文の入力を表す。いずれも原文を残して利用する関係を示す。図形と時間はRive、説明文と画面の設定はiOSのホストが管理する。
...
| [keyboard/scene.rml](keyboard/scene.rml)・[keyboard/rive.yaml](keyboard/rive.yaml) | 入力欄とキーボード、切替・項目選択・挿入の正本とCLI設定 |
| [keyboard-story.riv](../Nibble/Animations/keyboard-story.riv) | 本体Bundleだけへ同梱するキーボード説明の生成物。キーボード拡張へは追加しない |
...
接続名・型は[assets.json](assets.json)、初期値は各RMLのdefault instanceを正本とし、各ホストの契約と実バイナリを照合する。`motionAllowed`と配色はホスト→Rive、`active`はRive→ホストの出力である。`active`は演出の状態を表し、実際のコピー・保存・挿入の成否ではない。

| 用途・ホスト | Artboard / View Model | 配色 |
| --- | --- | --- |
| 保存・`AboutIllustration` | About / AboutStory | paper・ink・accent・muted |
| 入力・`KeyboardIllustration` | Keyboard / KeyboardStory | 上記4色にactionを加える。actionはキーボードの選択・入力先のcaret・挿入結果の青 |

両方ともState MachineはPresentation、default instanceはDefault。motionAllowedはBooleanで初期true、activeはBooleanで初期false、配色はColorで初期ライト色を持つ。SessionはホストViewのStateが所有し、ロード・再試行はattemptに結び付くtaskからawaitする。図の可視部分が10%未満ではpausedを渡し、Canvasが画面離脱とscenePhaseも照合する。

配色の制作初期値はライト用で、ホストがライト／ダーク外観へ合わせる。製品はコントラストを標準に固定する。アセット固有のタイムライン長・図形値は各scene.rmlで管理する。

## 演出の状態と時間

aboutの`CopyAndSave`は文章選択→コピー→複製の移動→保存→完了表示を繰り返す。keyboardの`SwitchAndInsert`は地球儀長押し→nibble選択→行タップ→本文挿入→結果保持を繰り返す。以下の表の「ループ」はそれぞれのタイムラインを指す。`Overview`は各完成図を示す。

| 入力・寿命 | Riveの状態と再生位置 |
| --- | --- |
| `motionAllowed=true`で開始 | ループを先頭から開始する。`active=true` |
| `motionAllowed=false`で開始、または再生中にfalseへ変更 | Overviewの完成図へ移る。`active=false` |
| `motionAllowed`をfalseからtrueへ変更 | ループを先頭から開始する |
...
用途に合うriveコマンドでライブプレビューを起動する。RMLの編集結果を確認して終了し、`build`で登録済みの配布物を生成する。`build`はCLI版とRMLの構造を照合し、`--verify`、`--once`によるunsigned生成を実行する。エラー・警告があると失敗し、成功時は配布先の`.riv`とmanifestのハッシュを更新する。
...
2. **静止画**：開始、選択、主動作、最大変形、収束を出力して開き、輪郭・配色・重なり・構図を確認する。keyboardでは切替メニュー、行タップ、挿入、結果保持、周期境界を含める。
3. **時間と状態**：複数周期を再生し、保存または入力の前後関係と周期境界を確認する。`motionAllowed`の初期値と途中変更を試し、`active`を読み戻す。
