...
すべて解除されると同じSessionの位置から進む。スクロールで見えなくてもViewは生存し得るため、ホストが可視性を判定して`paused`へ渡す。Reduce Motionはアセット固有の表示設定としてホストがData Bindingへ渡し、フレーム停止とは分けて扱う。
...
| 表示環境 | 配色、Reduce Motionの静止状態、スクロール可視性 |
