...
[Package.swift](Package.swift)はSwift tools 6.0とswift-app-macrosのexact指定を持つ。RiveRuntimeは[描画基盤](../../../runtime/rive/README.md)が固定ソースから生成するローカルPackageを参照する。生成先はリポジトリ内の`artifacts/RiveRuntime`であり、初回ビルドの前に`rive_runtime.py prepare`を実行する。AppMacrosのコンパイルにはSwift 6.3対応ツールチェーンを使用し、利用アプリはPackage.resolvedを共有する。接続にはApple runtime APIのWorker、File、Rive、ViewModelInstanceとData Bindingを使う。

別のアプリやリポジトリへ導入するときは、次の接続を用意する。

1. Rive描画基盤を同じ固定入力から生成し、Package.swiftのローカル依存先を接続する。Packageのproductを追加し、制作ソースと生成済み `.riv` を製品側で管理する。
