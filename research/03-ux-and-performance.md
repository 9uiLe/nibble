...
UIKitのCRUD・検索・コピーと保存失敗の試作は [E14〜E16](experiments/ios-26-5-validation.md)、実際の日本語かなキーボードの変換はE22に記録する。小画面・最大文字・キーボード併用では本文が切れたため、試作の縦配置は不合格とした（E24）。製品の最終UI・全アクセシビリティ条件・実機性能の比較は未実施。各結果から画面構成、入力部品、データの更新方式、性能予算を選ぶ。fixtureのビルド・入力反映・撮影と、製品の操作評価は検証範囲を分けて記録する。
