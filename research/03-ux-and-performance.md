...
「事実」は原著・Apple公式資料の内容、「推奨」はnibbleへの適用案、「未確認」は実装・実機・利用者による検証条件を示す。原著4本とApple一次資料のURL・閲読範囲・制約は資料台帳に記載する。外部本文は転載しない。
...
製品のUI・入力・アクセシビリティ・実機性能の試作は未実施。各結果から画面構成、入力部品、データの更新方式、性能予算を選ぶ。fixtureのビルド・入力反映・撮影と、製品の操作評価は検証範囲を分けて記録する。
...
- Altmann / Traftonの中断・再開研究は著者・出版社側の原文との照合が必要。採用する原著4本に含めず、作業再開の具体的な効果量の根拠にしない。
