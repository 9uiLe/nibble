...
確認日：2026-09-17。対象：nibbleの日本語iPhone UI、本体と共有拡張。最低対応OSはiOS 26.0、実行評価はiOS 26.5 Simulator。

この資料は、UIの目的・配置・操作を判断する一次資料の台帳である。[UI設計](../docs/design/README.md)が製品への適用を定義する。指針を満たすことと、nibbleの利用者にとって使いやすいことは別々に評価する。

## 根拠の扱い

- **指針**：Apple HIGなど、発行主体が推奨する設計。利用者実験による効果の実証とは区別する。
- **APIの事実**：公式のAPI説明と実装上の能力。特定のOSでの見え方は実行して確認する。
- **製品判断**：nibbleの用途に合わせた採用理由とトレードオフ。指針から自動的に導かれる唯一の答えとはしない。
- **仮説**：配置、寸法、文言、色などの比較が必要な選択。反証できる確認条件を付ける。

HIGは継続更新される。この確認日の内容には2026年の改訂が含まれるため、HIGの見た目や新機能をiOS 26.0で利用できるとは推定しない。新しいAPIは公式availability、対象SDK、ビルドで別に確認する。

Apple HIGは公式ページに対応する公開DocC JSONの本文を読み、共通節とiOS/iPadOS節を対象とした。他OS専用の数値や配置をiPhoneへ転用しない。WWDCは公式transcriptの記載範囲を読んだ。動画の連続視聴、掲載サンプルの実行はしていない。取得本文は無視対象の`artifacts/design-research/`に保存し、Gitには要約と出典を残す。
...
**適用**：一覧・設定・検索を領域、新規作成を操作、ピン留めを同じ一覧の対象変更として区別する。設定タブがnibbleで最適か、タブを常時展開するかは製品判断。
...
**適用**：保存済み本文・タイトルの検索へ即座に入力できる構成を選ぶ。検索履歴・候補表示は自動採用しない。下書きと削除項目を検索に含めるかは製品のデータ契約。
...
**適用**：タイトル→対象選択→内容の読み順、操作の下部配置、キーボードとの共存を別々に説明する。「すべてを親指に近づける」は本文の読み順を決める根拠にはならない。
...
**適用**：本文をカードへ全面展開せず、タイトルと短い要約を整列する。区切り線と共通背景を選ぶこと、2行に制限すること、ピンを先頭にすること自体はnibbleの判断。
...
**適用**：ルートの短いタイトル、編集の閉じる・保存、その他メニューを役割で分ける。タイトルの左寄せやサイズはブランドと可読性を比較して決める。
...
**適用**：操作位置は設定、フィルターは一覧に置く。文字サイズ、ダークモード、Reduce Motionをアプリ独自の設定として増やさない。
...
**適用**：表示シンボルと操作領域を分ける。新規作成56 pt、コピー44 ptという異なる役割の寸法は比較対象であり、Appleの指定値とは記載しない。新しいrole APIの採用根拠には使わない。
...
**適用**：長押し・スワイプだけをピン留めや削除への入口にしない。常設メニューボタンと編集画面の操作のどちらが適切かを比較する。
...
**適用**：作成・編集をシートにする。現在の「閉じる」は保存済み項目を変更せず下書きを保持する独自契約であり、通常の破棄を意味するCancelと同一視しない。スワイプ終了の無効化は保存契約に基づく例外として検証する。
...
**適用**：完全削除と下書き破棄に対象の分かる確認を置く。復元可能な削除には毎回確認を追加せず、回復手段を提示する。
...
**適用**：コピーの結果通知、削除直後の「元に戻す」、期限後の削除一覧を設ける。2秒・6秒は製品の調整値。単一削除の復元通知を汎用Undo/Redo履歴の実装とは呼ばない。
...
**適用**：読込や保存が完了する前に成功表示を出さない。小規模なローカル保存に架空の割合表示を追加しない。
...
**適用**：「保存」「下書きを破棄」「完全に削除」を区別する。ブランドの文章は空状態や製品情報に限定し、失敗時の説明を曖昧にしない。
...
**適用**：タイトル、本文、補足を意味で選ぶ。固定ptのアイコンや最大2行がすべての文字サイズに適切とは断定しない。
...
**適用**：文字の一覧面と浮く操作を分ける。クリーム色・錆色はブランド仮説。ガラスの背景合成を含め、実画面でコントラストを確認する。
...
**適用**：新規作成を全タブ共通のaccessoryにせず、一覧・検索の文脈で領域を確保する。
...
標準コンテナとtoolbar、検索role、検索欄の適用位置を使って新しい外観と振る舞いを構成する。外観を模写する独自バーより、標準部品の責務を活用する方針を採る。

**適用限界**：transcriptはnibbleの特定コードの動作保証ではない。採用するAPIのavailability、検索終了時の遷移、キーボードとの配置は別に確認する。

## 研究を運用に使う際の注意

操作時間、片手操作、中断からの再開に関する原著と条件は[UI/UXと性能](03-ux-and-performance.md)にある。古い端末、マウス、特定の参加者の実験値をnibbleのpt・秒・効果量へ換算しない。

「Hickの法則だからタブは3つ」「Fittsの法則だから56 pt」「黄金比だから余白24 pt」のように、一般則から製品の数値を直接正当化しない。数値には役割・制約・比較条件を付け、最適値が分からない場合は仮説と記す。

ユーザー提供の他アプリ画像はレイアウトの参照例として扱う。そのアプリの内部設計意図、利用者評価、API実装は画像だけから断定しない。個人情報を含む参照画像をリポジトリに転載しない。
