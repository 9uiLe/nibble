...
- Viewの比較ゲートはswift-app-macrosの`@Equatable`＋`EquatableBodyView`を使い、値型の`let`だけを入力にする。状態・操作は通常のViewに保持する。直接ゲート・`equatableBody`参照・手書き`==`・比較除外は禁止。値型・enumの標準Equatable合成は許可する。
