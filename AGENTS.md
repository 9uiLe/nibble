...
UIを構成・変更するときは[UI設計](docs/design/README.md)を読み、画面・部品の目的、配置理由、代替案、確認条件を記録する。外部指針、製品判断、未検証の仮説を区別し、既存要素には台帳のIDを使う。全体の情報構造や操作の意味が変わる場合は実装前に構成図と比較を作る。既存契約内の調整は理由と観測結果を残して進める。文書の整合は既存の文書検査、理由の妥当性はレビュー、実際の操作はローカルiOS検証で確認する。
