...
| 担当 | 責務 |
| --- | --- |
| 配布担当者 | Appleの契約、識別子、Capability、署名資産、API鍵の管理。輸出コンプライアンスの申告内容と保護されたログの確認。Apple側の処理完了・グループ配信の確認、実機での受け入れ |
| AIエージェント | ソースと配布コードの編集・検証、レビューした配布コマンドの実行、署名・アップロードの成否と公開manifestの確認、version・build番号の報告 |
...
通常の配布依頼に、エージェントによるApp Store Connectの画面確認は含めない。配信確認のためのサインインは依頼せず、送信結果が不明な場合のApple側の確認も配布担当者が行う。
...
| 受け入れ完了 | iOS 26.5実機で起動・編集・コピー・共有保存・キーボードの挿入と権限別コピーを確認した |
