...
一覧・検索・設定の画面タイトルは`toolbarTitleDisplayMode(.inlineLarge)`で上部のナビゲーションバー内に表示する。リストは`plain`スタイルで横幅を使い、スクロール内容の上余白を0にしてバーの直下から始める。Aboutは標準の戻る操作、削除一覧は閉じる操作を保つため、タイトルを`inline`とする。タイトルの表示方法は[Appleの定義](https://developer.apple.com/documentation/swiftui/toolbartitledisplaymode/inlinelarge)に従う。
