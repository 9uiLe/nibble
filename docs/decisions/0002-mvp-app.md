...
各フィルターは独立したButtonで、選択チェック・色とアクセシビリティの選択traitを持つ。操作領域は44 pt以上とし、大きな文字で幅を超える場合は横スクロールで全フィルターへ到達できる。
...
行をタップすると編集、コピーボタンを押すと本文をコピーする。可視の「その他」メニュー、長押しまたはスワイプでピン留め・削除を選び、スワイプし切るだけでは更新しない。アクセシビリティ文字では内容と行の操作を縦に分ける。削除・復元の通知と完全削除の確認には対象名を示す。コピーは触覚と通知で完了を伝え、通知本文をアクセシビリティのannouncementにも送る。
