...
一覧・検索・設定の画面タイトルは標準ナビゲーションバーの`topBarLeading`へ置き、スクロールしても同じ位置に保つ。`LibraryNavigationTitle`が標準タイトルの表示を除き、意味に応じた標準フォント`title3`の見出しを配置する。文字サイズの適用範囲は標準toolbarに従う。見出しは操作ボタンではないため、共有のガラス背景を付けない。リストは`plain`スタイルで横幅を使い、スクロール内容の上余白を0にする。行の背景は透明とし、一覧・検索・設定・製品情報・削除一覧の行と余白を共通の`nibbleCanvas`で統一する。行の境界は標準の区切り線で示す。一覧ではフィルターの直下、検索と設定ではバーの直下から始める。Aboutは標準の戻る操作、削除一覧は閉じる操作を保つため、タイトルを`inline`とする。タイトルの置換は[標準toolbar項目の制御](https://developer.apple.com/documentation/swiftui/toolbardefaultitemkind)を使い、独自のナビゲーションバーは作らない。検索入力中も[`searchPresentationToolbarBehavior(.avoidHidingContent)`](https://developer.apple.com/documentation/swiftui/searchpresentationtoolbarbehavior/avoidhidingcontent)で上部の見出しを保持する。検索欄と閉じる操作は標準検索UIに任せる。

「一覧」はタイトル直下にフィルターを固定し、その下のリストを縦にスクロールする。フィルターとリストは独立したViewとして配置する。各フィルターを個別のButtonとして公開し、選択した項目をアクセントカラーとアクセシビリティの選択traitで示す。操作領域は44 pt以上とし、大きな文字で横幅を超える場合は横スクロールできる。

| フィルター | 表示対象 |
| --- | --- |
| すべて | 先頭に下書きの再開、その下に保存済みの「ピン留め済み」と「その他」 |
| ピン留め | ピン留めした保存済み項目 |
| 下書き | 未完了の編集セッション |

「すべて」では該当する項目がない区分を表示しない。取得済みの1ページを区分へ一度だけ分配し、同じ項目を重複表示しない。並び順と取得上限は共通なので、ピン留め済みだけで上限に達する場合は「さらに表示」で続きを取得する。

フィルター変更時は取得上限とスクロール位置を初期化する。選択はタブ間の移動中は保持し、起動時は「すべて」に戻る。新規作成と一覧URLは「すべて」を選択し、保存または保持した下書きが見つかる状態にする。下書きの再開後に保存すると「下書き」の対象から外れる。

一覧と検索は独立した`LibraryModel`を持ち、検索語や取得上限を共有しない。保存層は共有し、画面の表示時と編集終了時に更新する。検索対象は保存済みの未削除項目全体とする。未入力または空白だけの検索語では検索の案内を表示し、入力した語に一致する項目を結果として表示する。削除した項目は設定から開く専用シートに置き、独立した標準検索欄で削除済みの項目だけを検索する。
...
| 空の一覧 | 作成の入口と製品の用途を表示 |
| 検索0件 | 語句を変える案内を表示 |
| 保存済み項目 | 行で編集、設定した側のボタンでコピー。長押しまたはスワイプでピン留め・削除 |
