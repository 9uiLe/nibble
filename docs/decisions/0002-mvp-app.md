...
| フィルター | 表示対象と順序 | 空の場合 |
| --- | --- | --- |
| すべて | 下書きの再開、保存済みの「ピン留め済み」、「その他」 | 用途の案内と作成の入口 |
| ピン留め | 未削除でピン留めした保存済み項目 | ピン留め方法と「すべてを見る」 |
| 下書き | 未完了の編集セッション | 下書きの説明と作成の入口 |

該当する項目がない区分は表示しない。保存済み項目の1ページをピン留め済みとその他へ一度だけ分配し、重複なく表示する。取得上限は両区分で共通であり、続きは「さらに表示」で取得する。

各フィルターは独立したButtonで、選択色とアクセシビリティの選択traitを持つ。操作領域は44 pt以上とし、大きな文字で幅を超える場合は横スクロールで全フィルターへ到達できる。
...
| 操作 | 見た目と操作領域 | 配置 |
| --- | --- | --- |
| 新規作成 | 56 ptの円形Liquid Glass、18 ptの＋ | 一覧・検索の下部、タブバーの上。設定した側に配置 |
| コピー | 16 ptのシンボル、32 ptの背景、44×44 ptの操作領域 | 保存済み項目の各行で設定した側に配置 |
...
行をタップすると編集、コピーボタンを押すと本文をコピーする。長押しメニューまたはスワイプでピン留め・削除を選び、スワイプし切るだけでは更新しない。コピーは触覚と通知で完了を伝え、通知本文をアクセシビリティのannouncementにも送る。
