...
「一覧」はタイトル直下に「すべて」「ピン留め」「下書き」のフィルターを固定する。選択した項目はアクセントカラーとアクセシビリティの選択状態で示す。フィルターは44 pt以上の操作領域を持ち、大きな文字で横幅を超える場合は横スクロールできる。「すべて」は「ピン留め済み」を上段、「その他」を下段に表示する。該当する項目がない区分は表示しない。取得済みの1ページを区分へ一度だけ分配し、同じ項目を重複表示しない。並び順と取得上限は共通なので、ピン留め済みだけで上限に達する場合は「さらに表示」で続きを取得する。下書きの再開は「すべて」の先頭と「下書き」に置く。「ピン留め」はピン留めした保存済み項目だけ、「下書き」は未完了の編集セッションだけを表示する。フィルター変更時は取得上限とスクロール位置を初期化する。選択はタブ間の移動中は保持し、起動時は「すべて」に戻る。新規作成と一覧URLは「すべて」を選択し、保存または保持した下書きが見つかる状態にする。下書きの再開後に保存すると「下書き」の対象から外れる。
