...
| `KeyboardModel` | 要求、取得世代、読込状態、ページ、結果文、実行中の操作IDを管理する。controllerへの副作用参照はweak。controllerの寿命中保持する |
...
`loadID`は読込要求の世代を識別する値である。要求変更と非表示で更新する。読込状態は`inactive / pending / loading(readID) / ready / failed(message)`の一つとし、進行表示・利用可否・失敗文を導出する。`readID`は同じ世代内でも読込開始ごとに新しく作り、最後に開始した実行だけが結果や失敗を反映できる。キャンセル要求と結果の有効性は別々に検査する。

| 状態・イベント | 表示と結果の採用 |
| --- | --- |
| 初回表示 | activeにし、新しい世代で先頭ページを取得。未取得を0件として表示しない |
| フィルター・ページ・更新 | 要求と世代を更新。取得済みページがあれば操作不可で表示し、読み込みを示す |
| 成功 | キャンセルなし、世代と実行IDが一致するloading状態だけが要求とページを反映。0件なら集合に応じた空表示 |
...
| 時点 | 挿入 | コピー |
| --- | --- | --- |
| 開始 | active、要求とページが一致、選択UUIDとrevisionが現在ページ内に存在、読込・失敗・別操作なし | 同左とフルアクセスあり |
