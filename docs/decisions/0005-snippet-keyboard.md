...
この画面が支える行動は「入力先を保つ → 保存済みの対象を見分ける → 本文を使う → 入力を続ける」である。一列の入力キーにタイトル・要約と「入力」の動作名を表示し、本文を直接挿入する。各キーの横に独立したコピーを置く。最上段で集合を選び、中央で対象を読み、最下段で結果と補助操作を確認する。
...
| 入力キー | 押すと保存済み本文を入力先へ挿入する | タイトルと本文先頭を各1行で表示する。末尾に常設する青い「入力」が、押した結果を明示する。動作名を淡い青の面で囲み、内容の識別と実行の意味を分ける。タイトル・要約・「入力」は一つのButtonであり、どこを押しても同じ本文を一度だけ挿入する。詳細への移動を意味する開示矢印は付けない。全文確認と編集は本体が担う |
| 項目名と要約 | 同名や似た内容から対象を見分ける | 名前をsubheadline、要約をcaption・secondaryにする。両方の始点を揃え、太字の反復を抑える。ピンの有無で文字位置を動かさない |
| ピンの記号 | 本体で固定した項目であること | 内容と「入力」の間へ小さいsecondary色の`pin.fill`を置く。入力内容に属する属性であり、独立した実行ボタンにはしない |
...
| `systemBlue`と10%の淡い面 | 「入力」の動作名だけに使い、主操作の場所と意味を示す。選択・成功・ピンには流用しない。動作名そのものが意味を持つので色だけに依存しない |
| primary / UIKitのlabel | 識別するタイトル、コピーなどの記号、操作結果。操作可能性は色だけでなく面・位置・押下表示で伝える |
...
| 「入力」の内側は左右10pt・上下8pt、角丸6pt | 2文字の動作名を省略せず、内容のテキストから区別する。独立したタップ領域を増やす値ではなく、入力キー内のラベルの余白である |
