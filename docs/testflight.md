...
依存解決前にRive描画基盤の生成物を照合し、必要なら署名なしでローカルビルドする。初回のこの工程には時間がかかる。再利用条件は[描画先の取得](architecture/presentation.md#描画先の取得)を参照する。
...
以下の初回設定を済ませ、リポジトリのルートでコマンドを実行する。

## 秘密情報の取り扱い

開発と配布は同じmacOSユーザーで行う。認証設定とAPI秘密鍵は`~/.appstoreconnect/`、署名の秘密鍵は配布担当者のKeychainで管理する。認証設定・秘密鍵・生ログはGitに保存せず、会話へ貼らない。

AIエージェントはこれらを直接参照せず、レビューした配布コマンドが返す工程・成否・公開メタデータだけを確認する。設定ファイルの作成・変更と生ログの確認は配布担当者が行う。これは同一ユーザー内の運用規約であり、OSによるアクセス分離ではない。別コマンドやコード変更による迂回もしない。
...
製品はAppleのData Protectionを使い、独自暗号実装を持たない構成として、`ITSAppUsesNonExemptEncryption`に免除対象外の暗号化なしを申告する。本体と両拡張の`Info.plist`に保存した値を完成archiveで検査してから送信する。暗号機能や依存の変更時は、配布担当者が申告の適合性を再評価する。エージェントやスクリプトは申告を推測して書き換えない。
...
配布の成立は、署名・送信、Apple側の処理とグループ配信、実機の動作を順に確認する。製品の受入はiOS 26.5 Simulatorで行う。配布担当者による任意の実機確認はiOS 26.5で、起動・作成・編集・コピー・共有拡張からの保存・本体での表示・キーボードからの挿入とコピーを対象とする。対象ビルド、端末とOS、操作、結果を記録する。キーボードは[操作検証](ios-verification.md#キーボードの操作検証)に従い、フルアクセスなしの挿入・コピー拒否と、許可時のコピーを別々に確認する。
