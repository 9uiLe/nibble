...
この記録は、共通の設計照合とnibbleの文書検査について、実装の責務・正当性・性能を確認した結果を示す。設計と公開Interfaceは[共通ツールキット](../tools/ui-design/README.md)、nibbleへの適用は[UI設計](design/README.md)に定義する。
...
内部の走査・解析は直列である。同じprocessからの独立した並行呼出しは4 worker・12呼出しで結果の一致を確認した。内部並列化によるスループット比較は未測定。共有parserや呼出しをまたぐ可変キャッシュを設けず、1回読取と表の順次走査を性能上の契約にする。

## 正当性と保証範囲

- 共通ツール35件、nibble側92件、計127件の回帰テストが成功した。
- 複数範囲に含まれるファイルも1回だけ開き、同じバイト列でhashと文書を判定する。大きなバイナリは分割して読み、末尾の変更も検出する。
- 未確認変更、対象の欠落、読取失敗、symlink、壊れたIDを拒否する。記録候補の参照一覧は呼出側の後続変更に影響されない。
- 文書の読取と見出し解析は、200リンクが同じ文書を参照しても各文書1回。別名で参照した文書も、配置固有のSwift規約を適用する。
- Nixの全6検査が成功した。文書・Swift規約・共通照合・基盤回帰を実行し、workflowとNix書式は同一入力の成功結果を再利用した。

対象は開発用の設計・照合基盤と接続処理である。iOSアプリの描画・応答・IPAサイズは今回測定していない。UI設計の理由の正しさ、利用者評価、更新せずに残された外部指針は機械検査の保証に含まない。検査中に別processがファイルを書き換えることを防ぐ全体ロックも提供しない。
