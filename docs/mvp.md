...
driverはダミー入力を下書きとして閉じて再開し、保存・コピー・日本語検索・タイトルと本文の編集・ピン留め・削除・復元・下書き破棄を操作する。実行ごとに一意な日本語タイトルを使い、検索で対象行を特定してコピーのUTF-8と復元後のUUIDを照合する。再開後と編集保存後のコピーが入力と完全一致し、破棄後に保存済み本文が保持されて下書きが一覧から消えることまで確認する。既存の項目は削除しない。行を開く前にキーボードを閉じ、一覧の配置が確定してから操作する。
