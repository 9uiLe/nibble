...
driverはダミー項目を作成し、コピー・日本語検索・編集画面・ピン留め・削除・復元・下書き破棄を操作する。実行ごとに一意な日本語のタイトルで項目を検索し、既存のピン留めや画面外の行と取り違えずに、コピーのUTF-8と復元後のUUIDを照合する。既存の項目は削除しない。行を開く前にキーボードを閉じ、一覧の配置が確定してから操作する。
