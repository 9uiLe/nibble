...
driverは上部の「すべて」「ピン留め」「下書き」を確認し、ダミー入力を下書きとして閉じて「下書き」から再開し、保存・コピー・日本語検索・タイトルと本文の編集・ピン留め・削除・復元・下書き破棄を操作する。実行ごとに一意な日本語タイトルを使い、検索で対象行を特定してコピーのUTF-8と復元後のUUIDを照合する。再開後と編集保存後のコピーが入力と完全一致し、破棄後に保存済み本文が保持されて下書きが一覧から消えることまで確認する。ピン留めフィルターで解除すると項目が消え、独立した検索から再度ピン留めすると戻ることも照合する。既存の項目は削除しない。行を開く前に標準キーボードの検索ボタンで入力を確定し、一覧の配置が確定してから操作する。標準検索欄の座標は毎回のAX情報から取得する。新規作成がタブバーの上にあり44 pt以上60 pt以下であること、入力中に隠れること、検索0件でも他のタブが絞り込まれないことを照合する。設定からnibbleについてを開き、新規作成・コピーの左右配置と再起動後の保持を照合する。設定から開く削除一覧では検索0件の案内、語句による絞り込み、同じUUIDでの復元と本文の完全一致を確認する。
