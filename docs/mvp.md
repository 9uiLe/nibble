...
- 入力キーのタイトル・本文・「入力」のいずれかをタップすると、保存済み本文を一度だけ直接挿入する。フルアクセスは不要。
...
2. iOS設定でnibbleを追加し、フルアクセスをオフにする。入力先でnibbleへ切り替え、入力キーの「入力」と本文領域をそれぞれ操作し、一度だけ挿入した結果を原文のUTF-8と照合する。
3. 既知の別文字列をクリップボードへ置く。コピーボタンで設定案内が出て、クリップボードと入力欄が変わらないことを確認する。
4. フルアクセスをオンにする。コピーした本文のUTF-8を照合し、入力欄への挿入を伴わないことを確認する。
