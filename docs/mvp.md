...
本体の標準UI driverとは別に、他アプリを入力先にして次を確認する。端末・OS・ビルド・本文・権限・入力先アプリを記録し、[証跡の手順](review-evidence.md)に従って操作・画像・録画を保存する。具体的な実施結果は[キーボードの検証](keyboard-validation.md)を参照する。
...
3. 「…」で全文を開き、誤挿入がないこと、長文末尾、戻った位置を確認する。既知の別文字列をクリップボードへ置く。詳細のコピーボタンで設定案内が出て、クリップボードと入力欄が変わらないことを確認する。
...
| キーボード拡張 | フルアクセスなしで挿入し、許可時だけコピーできる。フィルター・ページ・更新・標準キーボードへの復帰を操作できる |
