...
小画面、ライト/ダーク、画面外へのスクロールと復帰、停止中の配色変更、開き直し、背景復帰、Reduce Motionを初期から有効にした場合と表示中の変更を含める。説明文のアクセシビリティ情報と、VoiceOverの音声・フォーカス順の確認は区別する。読込失敗は専用Simulatorへ同梱したkeyboard-story.rivを一時的に退避するなど、明示した障害条件で確認し、復元して「説明アニメーションを再読み込み」から回復する。製品の制作ソースや配布物へ障害を残さない。 Issue #34の観測・媒体・容量と負荷は[検証記録](keyboard-guide-animation-validation.md)を参照する。
