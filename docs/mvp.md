...
| 入口 | 期待する内容 |
| --- | --- |
| 一覧→すべて | 最新1件の下書き再開帯と件数付きの全下書きへの入口、使用回数順の保存済み。最終使用から720時間以上なら未使用の補足を表示する |
...
| 操作 | 手順・結果 |
| --- | --- |
| 作成 | 空一覧の「新しく作る」、または一覧・検索画面下部の＋から本文を入力して保存する。タイトルは任意。＋は新しい下書きを作る |
...
driverは20.3秒の連続再生区間を録画し、`loop-frames/`へ時刻付き静止画を抽出する。本文全体はライトで一度たどり、同じ末尾位置と先頭をダークで撮影する。標準の戻ると表示設定の固定は`check-interface-ui.py`が確認する。driverは設定から説明画面を開き、自動再生、ライト／ダーク、背景復帰、表示中のReduce Motion切替を画像と録画に記録する。キャプション、再生ボタンがないこと、末尾段落への到達を自動判定し、図の描画と動きは媒体を開いて確認する。`--interruptions`はタブ往復、図が見えている間のドラッグと減速、画面外との往復、画面外のまま背景から復帰する代表条件を加える。反復と30秒の背景滞在は`--stress`で選ぶ。Reduce Motionが初期から有効な条件は、各コマンドに`--reduce-motion enabled`を加えて別に実行する。
...
キーボード利用案内S10のC49は、設定の「nibbleキーボード」から別に確認する。導入文の次に図と説明があり、地球儀からの切替、行タップ、入力先への本文挿入を順に示すこと、保存済み行が残ることを確認する。8.4秒の周期を2周以上録画し、切替前、行タップ、挿入、結果保持、フェードと次周期を開いて確認する。図へのタップで実際の入力・コピー・保存が起きないこと、全段落と標準戻るへ到達できることも確認する。
...
driverはダミーデータを使い、次の契約を実際の画面操作で確認する。通知からの取り消しと検索入力の保持は`check-notice-ui.py`、説明画面を含む最大文字サイズとの比較は`check-interface-ui.py`が担当する。
...
| 下書きの破棄、削除一覧からの復元 | 破棄しても保存済み本文を保持し、削除一覧の検索・復元で同じ項目を利用できる |
