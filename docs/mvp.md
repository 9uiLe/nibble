...
フィルターはスクロール中も上部にあり、大きな文字では横スクロールして選ぶ。選択はタブを移動しても保持し、起動時・一覧からの新規作成・一覧URLでは「すべて」に戻る。検索語と一覧フィルターは独立している。
...
| アプリ情報 | 設定→nibbleについてで使い方とデータの扱いを確認する。説明イラストは、ほかのアプリの文章をコピーしてnibbleへ保存する流れを自動で繰り返す。再生・停止ボタンはなく、Reduce Motionでは完成図を示す |
