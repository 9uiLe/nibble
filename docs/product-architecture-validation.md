...
比較元は`3ce14844af5b32793926468a753099038a8482cf`、検証対象の製品・テスト・driverは`c84280c3c26b6cf0a8cbf75dd46e8d3ee78e40e2`。目的は保存・検索・下書きの整合性と画面操作を保ち、保存資源とOS操作の責務を限定し、配布容量を抑えること。構成の正本は[製品仕様・要件](product-specification.md#構成と責務)、実装契約は[ライブラリ規約](library-policy.md)とする。
...
現在の`benchmark-store.py`は両版を`-Osize`で比較する。[実行手順](performance-verification.md#保存層の比較)は現在の保存層比較に使い、この記録の最適化条件を再現するコマンドとは扱わない。
