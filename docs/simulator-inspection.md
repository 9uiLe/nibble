...
この文書は、保存済みデータを扱う`inspect_ui.py`の公開インターフェースと開発方法を定義する。原本の取得は[iOS実行手順](ios-verification.md#個別の画面読取と操作)、観測内容の記録は[証跡手順](review-evidence.md#画像動画のレビュー記録)、責務と保証範囲は[基盤設計](architecture/verification.md#画面の観測と閲覧用データ)を参照する。
