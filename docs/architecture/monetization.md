...
変数の構文と展開は[SnippetVariables.swift](../../app/Shared/Domain/SnippetVariables.swift)が所有する。編集面は既存名の再利用と新規名の作成を分けて提示し、どちらも本文末尾に同じ印を追加する。コピーとキーボード挿入は同じ名前に一つの値を要求し、全出現箇所を利用時の出力だけで置き換える。保存層は原文をそのまま保持する。
