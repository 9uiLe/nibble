...
3. コピーまたはキーボード入力を選ぶと、対象の項目名、名前ごとの「差し替える内容」と印の説明・入力欄、入力前の原文を示す。値が全て揃うと原文表示を完成文に切り替える。省いた文章は「全文を確認」で開ける。
4. 通常の確定操作は入力欄・文章と別の下部位置に保つ。入力欄にフォーカスがある間は文章の直後へ移し、キーボード表示でシートが広がっても操作の順序と近さを保つ。未入力時は利用不可を輪郭と文字で示し、値が揃ったら強調する。閉じる操作を含む確定前の取消ではコピー、挿入、使用記録を行わない。確定後の出力だけに値を適用し、保存済み本文は変えない。

本体とKeyboardは[VariableFillView](../../app/Shared/Interface/VariableFillView.swift)を共有する。本体はシート、Keyboardは入力面内に表示する。表示可能な高さが異なるため、入力欄と文章はスクロールし、通常の確定操作は画面下部に置く。入力中は文章と確定操作を同じスクロール領域に置く。本文が160文字または4行の表示範囲を超える場合は短く示し、全文への入口を付ける。項目名と原文を先に示すことで、利用者が差し替え先を確かめてから値を確定できる。
