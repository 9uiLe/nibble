...
1. 本文のカーソル位置または選択範囲を決め、「変数を追加」で既存名の追加行か新規名の作成を選ぶ。両方法とも同じ挿入規則を使い、選択中の文字は印に置き換わる。本文に位置がない場合は末尾に加え、追加後のカーソルは印の直後へ置く。シートの「追加せず閉じる」では本文を変えず編集画面に戻る。
...
4. 通常の確定操作は文章の直後に置き、文章とともにスクロールさせる。本体シートで入力欄にフォーカスがある間はキーボード上のセーフエリアに固定し、スクロール領域と重ねない。未入力時は利用不可を輪郭と文字で示し、値が揃ったら強調する。閉じる操作を含む確定前の取消ではコピー、挿入、使用記録を行わない。確定後の出力だけに値を適用し、保存済み本文は変えない。
