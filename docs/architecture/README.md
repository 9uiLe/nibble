...
| `app/Nibble/Navigation/` | `LibraryRootView`のタブ・シート・scene、`AppRoute`、共通の`ScreenHeading`、タブ画像 |
...
| `app/Shared/Domain/` | 項目、下書き、一覧要求・結果、保存操作のprotocol |
| `app/Shared/Persistence/` | SQLite接続、schema、同期SQL、`SnippetStore` actor |
| `app/Shared/Editing/` | `EditorModel`と`SnippetEditor`。本体と共有拡張で使用 |
| `app/Shared/Keyboard/` | `KeyboardModel`と既存DB専用の`KeyboardReader` |
...
```mermaid
flowchart TD
    App[NibbleApp] --> Root[LibraryRootView]
...
`LibraryRootView`は一覧・検索それぞれのモデル、選択タブ、検索フォーカス、編集シートを保持する。scene・タブ・シートの状態が通知の表示資格を決める。通知は成功イベントと対応し、Viewの再生成では成功を発火しない。[通知の契約](../design/rationale/result-notices.md)に表示と振動の寿命を定める。
...
`scripts/verification_catalog.py`が検証工程のID・実行コマンド・期待scheme・runの種類を定義し、`verify.py`が差分から選択して直列実行する。製品回帰、基盤fixture、性能測定は目的で分ける。`validation/`は基盤試験と製品の保存層測定・共有入力に必要なfixtureだけを持つ。
