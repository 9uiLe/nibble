...
Markdown編集は原文のUTF-8、装飾範囲、選択、IMEの未確定文字、マウント時のフォーカスをhostedテストで確認する。本文の解析番号と下書きの保存番号は別に検査し、タイトルの変更や完了後の入力で不要な解析を始めないことを保証する。ソフトウェアキーボードとの実際の重なりはOS導線で確認する。
...
| `controls-ui` | 検索への移動時の非フォーカス、設定のバージョン・削除一覧、編集の操作領域と排他、補足からのフォーカス復帰、呼出元の保持 |
