...
Markdown編集は原文のUTF-8、装飾範囲、選択、IMEの未確定文字、マウント時のフォーカス、プレビュー往復での入力View・選択の保持をhostedテストで確認する。プレビューの記号除去・見出しと強調の書式・段落とリストの区分を解析結果で確認し、保存後は原文のUTF-8を照合する。LF・CR・CRLFとその混在、NUL、結合文字・絵文字を含む原文でも、装飾が意図した文字へ適用されることを検査する。本文の解析番号と下書きの保存番号は別に検査し、タイトルの変更や完了後の入力で不要な解析を始めないことを保証する。ソフトウェアキーボードとの実際の重なりはOS導線で確認する。
...
| `controls-ui` | 検索への移動時の非フォーカス、設定のバージョン・削除一覧、編集の操作領域と排他、タイトル・本文を保持した補足往復とフォーカス復帰、呼出元と一覧フィルターの保持 |
