...
Markdown編集は原文のUTF-8、装飾範囲、選択、IMEの未確定文字、マウント時のフォーカスをhostedテストで確認する。LF・CR・CRLFとその混在、NUL、結合文字・絵文字を含む原文でも、装飾が意図した文字へ適用されることを検査する。本文の解析番号と下書きの保存番号は別に検査し、タイトルの変更や完了後の入力で不要な解析を始めないことを保証する。ソフトウェアキーボードとの実際の重なりはOS導線で確認する。
