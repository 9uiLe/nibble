...
端末の排他は`ios.simulator_lock`、runのログ・媒体・ソース照合と成否は`ios.Run`、OSの編集メニューを経由した入力の確定は`Run.paste_text`、本体エディターのフォーカスと置換は`product_ui.py`が所有する。UI driverは到達したい状態のpredicateを`Run.wait_ui`へ渡す。空の遷移状態だけを有限回待ち、ツールの失敗や対象processの終了はその場で失敗にする。画面遷移の待機は例外の表示文字列から再試行の可否を決めない。固定版sim-useが日本語編集メニューを選べない場合は、実際に観測したOSメニューの項目を操作し、入力結果を確認する。
