...
端末の排他は`ios.simulator_lock`、runのログ・媒体・ソース照合と成否は`ios.Run`、OSの編集メニューを経由した入力の確定は`Run.paste_text`、本体・共有エディターのフォーカス・表示切替・置換は`product_ui.ProductRun`が所有する。UI driverは到達したい状態のpredicateを`Run.wait_ui`へ渡す。空の遷移状態だけを有限回待ち、ツールの失敗や対象processの終了はその場で失敗にする。画面遷移の待機は例外の表示文字列から再試行の可否を決めない。置換は長押しで開いたOSの「すべてを選択」と「カット」を観測して操作し、ペースト後の入力値を確認する。検証用の入力例は短文で、長押しできる行末の空間を持つ。フォーカスによるスクロール後は欄と固定操作の矩形を再取得し、可視領域を使う。

製品の標準タブは`product_ui.ProductRun`が操作する。OSがタブのaccessibility識別子を返さない場合は、nibbleの画面内で観測したRadioButtonの役割と表示名を照合し、その位置をタップする。重複した候補や非表示のタブは失敗とする。保存するAX原本へ識別子を補わず、検証用のselectorと観測した要素の対応を`native_tabs`で扱う。
