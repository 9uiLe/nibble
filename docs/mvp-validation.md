...
検証は2026-09-13〜2026-09-15に実施した。各結果は記載したソースに対応する。runは1回の検証実行を識別する名前であり、`artifacts/ios/<run>/manifest.json`にコミット、作業ツリーの状態、対象ファイルのSHA-256、端末、コマンド、終了コードを保存する。未コミット状態を含むrunはファイルのSHA-256で対象を特定する。
...
`16868e6`は、モデルの操作を完了までawaitし、UI所有者がTaskingの開始・寿命・重複方針を管理する製品ソースである。下書き書込と通知期限はSwiftUI `.task`から直接awaitする。

| 検証 | 結果 | 詳細 |
| --- | --- | --- |
| 製品Releaseテスト | SEで26件成功、失敗・skipなし | APIの直接await、snapshot保存、即時保存・閉じる、通知期限、所有者の寿命・重複を含む |
| Debug・Releaseのビルドと実行 | 本体・共有拡張で成功 | Debugは17 Pro、ReleaseはSEのテストと17 Proの基本操作で確認 |
| 基本操作・下書き・共有 | コピーのUTF-8、復元UUID、入力直後に閉じた下書きの再開・保存、Safari共有を確認 | 17 Pro、標準文字サイズ、ライト外観 |
| 通知・再起動 | Reduce Motion有効・無効で通知更新・消去・編集開閉、各5回のDebug再起動と2回のRelease再起動を確認 | 表示設定は無効へ復元 |
| 機械検査 | Nix 4 check、Python 33テスト、17 Swiftソースの規約検査が成功 | 構文境界・別名化・壊れた構文の回帰テストを含む |

run ID、診断、検索測定、照合に失敗した条件と再実行の結果は[非同期API境界の検証記録](async-policy-validation.md)に記載する。画像5点・動画6本は[PR #7](https://github.com/9uiLe/nibble/pull/7)に添付している。製品テストと撮影時の全`app/`ファイルのSHA-256は`16868e6`と一致する。`89cf8ff`はLintとその回帰テストだけを変更している。

## 証跡の参照方法

結果は実施日と対象ソースの組み合わせで参照する。2026-09-14の`9686f3a`はTasking・ScopedAnimationの所有・寿命・表示範囲を検証した構成であり、モデルのawait可能な操作APIとUI所有者の構成は`16868e6`を参照する。

2026-09-14の詳細と画像4点・動画5点は[Tasking・ScopedAnimationの検証記録](library-policy-validation.md)と[PR #7](https://github.com/9uiLe/nibble/pull/7)、2026-09-13の表示・入力・OS連携の画像6点・動画6本は[PR #6](https://github.com/9uiLe/nibble/pull/6)にある。ローカルの生ログ・画像・動画はGit管理対象外で、新しいcheckoutには含まれない。

以下は日付・ソースごとの観測を保存する記録である。異なるソースの検証を組み合わせて、同一構成で全条件を実施済みとは判断しない。

## 2026-09-14のビルドと自動テスト
...
## 2026-09-13の表示・入力・OS連携

以下は2026-09-13に実施した条件の記録である。記載したソースでの観測を示す。対象ソースを明記していないrunはmanifestのコミットとファイルSHA-256を参照する。
...
- 2026-09-13の表示・IME・URL・ペーストの観測は、対応するソースと条件に限る。`9686f3a`・`16868e6`での確認項目との全組み合わせを検証したものではない。
