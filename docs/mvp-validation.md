...
最終製品ソースは`f14c5008008b44ea6ca25c67ceeb685775ec0522`。[SwiftUIの調査・検証](swiftui-investigation.md)に、全ターゲットの棚卸し、状態・Taskの所有権、キャンセルと後着結果・編集提示の寿命の修正、UIKit比較、未確認条件を集約する。

- Release 72テスト成功。取消後の読込表示、旧編集要求の成功・失敗、取消直後の再入場を制御した回帰テストを含む。
- 本体の標準操作と、一覧・検索の編集途中の背景復帰を最終コードで確認。原文のUTF-8と復元したUUIDを照合した。iPhone SEで固定表示設定、長いタイトル・120行本文とキーボードの背景復帰も確認。共有拡張・モデル測定の対象ソースは詳細記録で区別する。
- 同条件の1万件モデル計測で大きな負荷増加は観測しなかった。Instrumentsは記録開始に失敗し、描画速度やfpsの改善は未評価。
- 原本画像と動画の抽出フレームをローカルで観察。全編再生とPR添付・閲覧確認は未実施。
