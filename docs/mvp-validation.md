...
入力キーと独立コピー、標準セグメント、下部の結果・補助操作で構成するUIは`740909e72e844c36f452c5ea9a19c2be5a14a7b5`で評価した。ReleaseのiOS 26.5 Simulatorで縦のライト／ダーク、選択、原文挿入・権限別コピー、更新・終了を確認し、原本PNG8枚と録画の抽出フレームを閲覧した。Nix全7検査が成功した。[入力キー構成の検証](keyboard-validation.md#入力キーを主操作にした構成)にrunと未確認条件を記載する。今回の構成の横向き・狭幅・複数ページ・性能とTestFlight配布は未確認である。
