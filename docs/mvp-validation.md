...
ReleaseのSwiftテスト82件（パラメーター展開後94件）、本体の標準UI操作、Nix全7検査が成功した。iOS 26.5 Simulatorで権限なしの直接挿入、許可ありのコピー、縦横と配色、標準キーボードへの復帰を確認した。実機性能・Apple側の登録・署名配布は未確認で、画像と録画はローカル保存である。
