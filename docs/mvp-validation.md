...
ReleaseのSwiftテスト82件（パラメーター展開後94件）、本体の標準UI操作、Nix全7検査が成功した。iOS 26.5 Simulatorで権限なしの直接挿入、許可ありのコピー、縦横と配色、標準キーボードへの復帰を確認した。`0.1.0 (202609181319)`のTestFlight向けアップロードも成功し、詳細を[配布の検証記録](testflight-validation.md#キーボードを含む署名済みipa)へ記載した。Apple側の処理・グループ反映と実機性能は未確認で、画像と録画はローカル保存である。
