...
[キーボードの評価記録](keyboard-validation.md)に、読み取り専用の保存領域、挿入・コピー、権限と操作寿命、OS統合の確認範囲を記録する。

入力キーへ「入力」の動作名を表示するソース`a09c939bc7982cd036cbe37b726ff5bcc7b08c81`は、ReleaseビルドとNix全7検査が成功した。Simulatorのソフトウェアキーボードを表示できず、新しいラベルの実画面・タップ・録画確認は未完了である。[動作名の検証記録](keyboard-validation.md#入力の動作名)に原因と再開手順を記載する。

ラベル追加前の入力キーと独立コピー、標準セグメント、下部の結果・補助操作は`740909e72e844c36f452c5ea9a19c2be5a14a7b5`で評価した。ReleaseのiOS 26.5 Simulatorで縦のライト／ダーク、選択、原文挿入・権限別コピー、更新・終了を確認し、原本PNG8枚と録画の抽出フレームを閲覧した。[入力キー構成の検証](keyboard-validation.md#入力キーを主操作にした構成)にrunと未確認条件を記載する。新ラベルの確認へ旧runを流用しない。横向き・狭幅・複数ページ・性能とTestFlight配布も未確認である。

初期統合ではReleaseのSwiftテスト82件（パラメーター展開後94件）、本体の標準UI操作、Nix全7検査が成功した。iOS 26.5 Simulatorで権限なしの直接挿入、許可ありのコピー、縦横と配色、標準キーボードへの復帰を確認した。`0.1.0 (202609181319)`のTestFlight向けアップロードも成功し、詳細を[配布の検証記録](testflight-validation.md#キーボードを含む内部配布)へ記載した。Apple側の処理・グループ反映と実機性能は未確認で、画像と録画はローカル保存である。
