...
入力キーへ「入力」の動作名を表示するソース`a09c939bc7982cd036cbe37b726ff5bcc7b08c81`は、ReleaseビルドとNix全7検査が成功した。表示環境の復旧後に縦のライト／ダーク、ラベル・本文からの一度の挿入、93 byteの原文一致、コピー拒否、閉じる・再表示を実操作と画像で確認した。録画は指定3時刻の抽出フレームを確認した。[動作名の検証記録](keyboard-validation.md#入力の動作名)にrun、表示復旧の経緯、未確認の条件を記載する。
