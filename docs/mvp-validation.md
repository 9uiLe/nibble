...
[UI改善の検証](ui-ux-validation.md)は、可視の整理メニュー、下書きの識別と表示上限、文字拡大、検索の失敗と回復、対象名付き復旧、編集の説明を対象とする。対象ソース・端末・実操作・性能の値と制約は同記録を正とする。設計理由は[採用判断](design/decisions/0001-library-actions.md)に定義する。
