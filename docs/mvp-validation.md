...
| 対象 | 確認したことと範囲 |
| --- | --- |
| 採用レイアウト | `eb0ebdb461fd3e06dc6df3dfb10385e866a7ecc9`のRelease runで縦のライト／ダーク、ラベル・本文からの一度の挿入、93 byteの原文、コピー拒否、閉じる・再表示を確認 |
| 製品テスト | `c07902e25d5490ee4b1a1d78c63d15780a27e847`で82件・パラメーター展開後94件成功。採用レイアウトでの全件再実行は未実施 |
| OS連携と本体 | 検証記録のソース別runで許可ありコピー、フィルター、標準キーボードへの復帰、利用案内、本体標準UI driverを確認 |
| 媒体 | [PR #27](https://github.com/9uiLe/nibble/pull/27)へPNG3枚と録画1本を添付。原本画像・指定3時刻のフレームを閲覧し、未ログインのブラウザーで公開先の読込を確認 |
...
| 未確認 | 採用レイアウトの横向き・狭幅・0件・複数ページ、許可ありコピーの再確認、VoiceOver音声、性能。Apple側の処理・グループ反映と実機操作も未確認 |

別レイアウトの縦横確認や機能テストを、採用レイアウトの全条件の合格へ読み替えない。PRは追加評価を要するDraftである。
