...
この文書は2026-09-13〜16に実施した製品の検証を、対象コミットと条件とともに保存する。App Groupは`group.dev.nibble.app`の構成を含む。各観測は記載したソースにだけ適用し、現在の設計の実施状況は[検証結果の索引](mvp-validation.md)から参照する。

機能と責務の正本は[製品設計](decisions/0002-mvp-app.md)。テスト件数、API世代、表示設定、識別子の異なる結果を合成して一つの構成の合格とは扱わない。
