...
上段は「すべて／ピン留め」と更新（C45）、中央はタイトル・要約・挿入・コピー（C46）、下段はページ・切替・終了・結果（C47）とする。対象集合を選んでから内容を読む順序を保ち、限られた高さの伸縮を一覧へ割り当てる。背景はOSのkeyboardスタイルに委ね、ホストViewを透明にする。文字と操作はニュートラルなシステム色とし、OSのキーボード周囲との連続性を保つ。
