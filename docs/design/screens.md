...
**目的**：他アプリの入力欄を保ちながら保存済み本文を識別し、挿入またはコピーする。

| 領域 | 構成と配置理由 |
| --- | --- |
| 上段・固定 | C45の「すべて／ピン留め」を全幅の標準segmented Pickerで選ぶ。同じ一覧への排他的な条件を、対象を読む前に確認できる |
| 中央・可変 | C46の一列の入力キーと独立コピーをスクロールする。タイトル・要約の始点を揃え、末尾のピンと青い「入力」で属性と操作を分ける |
| 下段・固定 | C47の結果文、必要なページ操作、更新・終了を置く。OSが要求する場合だけ切替キーを先頭に置き、一覧末尾へ移動せず操作できる |

背景はOSのkeyboardスタイル、ホストViewは透明、入力・コピーは公開システム色のキー面とする。Pickerの外周、一覧の左右・下部、キー間は6pt。入力キーは48pt以上、コピーは44×48pt、下段の操作は44pt以上を確保する。色・形・寸法の役割は[技術仕様のレイアウト](../decisions/0005-snippet-keyboard.md#レイアウトと操作の意味)に定義する。

タイトル・要約・「入力」は一つのButtonで、同じ本文を一度だけ挿入する。コピーは別ボタンで、フルアクセスなしの場合は設定案内を表示する。対象取得中は取得済みページを操作不可で保持し、空表示は読込完了後に確定する。1ページだけならページ番号・矢印を省き、長い結果文には折り返しと中央の高さ調整を使う。

**確認条件**：原文一致、権限別操作、ページ境界、変更・削除の拒否、本文取得中の入力先変更・権限取消・非表示を確認する。0件・51件以上・長文・縦横・狭幅・ライト／ダークで内容と主要操作へ到達できることを確認する。同じ入力先で標準キーボードと切り替え、背景・選択・文字・操作の識別を評価する。
