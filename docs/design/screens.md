...
| 使う流れ | C44の説明イラスト、短い文章、再生操作 | 導入の直後でコピーと入力先の関係を示す |
...
C44は本文と一緒にスクロールする400:208比率のイラストで、文章の最大幅の内側へ収める。図、短い説明、再生ボタンの順に置き、演出中も本文を読み進められる。4秒の一回再生後とReduce Motionでは完成図を示す。図は読み上げとタッチの対象から除き、隣接する説明と標準Buttonが意味と操作を伝える。読み込み中・失敗時も文章を保持する。停止・復帰と外観の契約は[演出設計](../decisions/0004-rive-presentation.md)に定義する。
