...
タイトルとフィルターは現在地と集合を保持し、内容は縦にスクロールする。下部の作成はF06のsafe-area領域、一覧・検索の通知は上部safe-areaバーへ置く。[通知の設計](decisions/0004-tab-accessory-notices.md)に従い、検索語・フォーカス・スクロールの状態を保ち、タブ移動・シート表示・背景移行で通知を終了する。末尾の項目をボタンの上まで移動できることを評価する。

一覧・検索は、検索入力中も含めて通知を上部へ統一する。通知表示中はその高さだけ内容領域が縮むが、タブと作成ボタンの位置・幅は維持する。通知消去後のスクロール位置を確認する。採用理由は[通知の設計](decisions/0004-tab-accessory-notices.md)、解決対象は[G13](audit.md#g13-b--検索入力中の通知配置)・[G14](audit.md#g14-b--一時通知によるタブバーの再配置)を参照する。
