...
flowchart TB
    ...
    M --> C[画面下部：新規作成 C09]
    C --> N[タブのaccessory：通知・取り消し C18・C19]
    N --> T[標準タブ C01]
```

操作結果は短時間の通知で伝える。削除通知は対象名と「元に戻す」を持ち、タブ移動・シート表示・アプリが非アクティブになると終了する。通知内容と寿命、検索入力中の未決定条件は[操作完了通知の設計](decisions/0004-tab-accessory.md)に定義する。
...
タイトルとフィルターは現在地と集合を保持し、内容は縦にスクロールする。新規作成は画面内のF06の確保領域へ、通知は標準タブのaccessoryへ独立して置く。通知がない間はaccessoryの領域を解放する。末尾の項目をボタンの上まで移動できることを評価する。
...
検索のコピー・削除・復元はC18/C19の通知を使う。検索入力中は結果の可読性と取り消し操作、検索語とフォーカスの保持を必須条件とする。この状態での通知配置は未決定であり、[通知の設計と受け入れ条件](decisions/0004-tab-accessory.md#未決定条件と受け入れ)に従う。
