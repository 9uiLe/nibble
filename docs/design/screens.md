...
flowchart TB
    ...
    F --> D[スクロール：下書き C07]
    D --> O[スクロール：使用回数順の保存済み C05・C06・C10]
    O --> M[内容末尾：追加取得・読込 C08・C16]
    M --> C[下部：新規作成 C09]
    C --> T[標準タブ C01]
    N[専用ウィンドウ：通知・取り消し C18・C19] -.上部へ重ねる.-> H
```

図は「すべて」の構成。空の区分は表示しない。通知は内容の外側のウィンドウに属し、表示の有無で内容領域を増減させない。共通背景を使い、区分は見出し、隣接する項目は区切り線で識別する。ピン留めと削除一覧は単一の集合であり、画面名と重複する区分見出しを置かない。

| フィルター | 内容 | 選択変更の結果 |
| --- | --- | --- |
| すべて | 直近3件の下書きと全下書きへの入口、使用回数順の保存済み | 取得上限と縦スクロールを初期化する |
...
保存済みは使用回数の降順、更新日時の降順、IDの昇順で全体を並べてから取得件数を制限する。ピン留めは絞り込みに使い、「すべて」では別区分を作らない。検索にも同じ順序を使う。使用とは本体の一覧・検索で保存済み本文のコピーが完了することを指し、コピー後に順位を再取得する。

最終使用から720時間以上経過した保存済み行には「30日以上未使用」と最終使用日を補足する。履歴がない項目には表示しない。ピン留めも対象に含め、削除は利用者が判断し、行の削除操作・通知の取り消し・削除一覧からの復元を使う。条件と再評価は[使用履歴の設計](decisions/0003-usage-order.md)に従う。

タイトルとフィルターは現在地と集合を保持し、内容は縦にスクロールする。下部の作成はF06のsafe-area領域を使い、末尾の項目をボタンの上まで移動できるようにする。

コピー・削除・復元の結果は、元画面と同じシーンの専用UIWindowで上部へ重ねる。通知は内容のsafe areaを変えず、表示前・中・消去後で一覧・フィルター・タブ・作成ボタンの位置を維持する。通知カードの外側のタッチは背面へ届く。通知はタブ移動、シート表示、シーンの非active化で終了する。結果文、2秒・6秒の期限、対象を限定した取り消し、入力とウィンドウの責務は[通知の設計](decisions/0004-result-notices.md)に定義する。
...
| 領域 | 構成 | 配置理由 |
| --- | --- | --- |
| 上部 | C02「検索」。成功時は専用ウィンドウのC18・C19を重ねる | 入力中も結果と取り消しを同じ位置で示し、検索欄と分離する |
...
一覧の選択集合と検索状態は独立させる。検索0件でも一覧の内容は変わらない。結果通知はキーボードの表示状態で配置を変えず、通知の表示と取り消しで検索語・フォーカスを維持する。検索欄、読み込み、結果、失敗の関係を明確にし、必要な回復操作へ到達できることを評価する。
...
**目的**：タイトルから保存済み本文を選び、他アプリへ一度で挿入する。全文確認・コピー・ピン留めは詳細で行う。

| 領域 | 構成と配置理由 |
| --- | --- |
| 上段・固定 | C45のコンパクトな「すべて／ピン留め」と更新。対象選択を一覧の前に置く |
| 中央・可変 | C46の角丸リストにタイトル主役の行と1行プレビュー、薄い区切り線。タイトル領域は挿入、右端の「…」は全文と操作 |
| 詳細 | 一覧を保持したまま同じ領域へ切替。戻る・ピン、スクロール全文、コピー・強調した入力の順 |
| 下段・固定 | C47の控えめなブランドと短い結果、必要なページ操作、終了。OSが要求する場合だけ地球儀を表示 |

基準高は288pt、vertical size classがcompactの場合は196ptとし、OSの必須制約を優先する。幅と中央のスクロール領域は利用可能な表示範囲に合わせる。地球儀はOSが要求する場合にだけ提供し、システムの音声入力・ホームインジケータ・セーフエリアを独自の部品や余白で複製しない。各操作は44×44pt以上のタップ領域を持つ。文字サイズはlarge、太字・コントラスト設定は標準値に固定し、ライト／ダーク外観には追従する。色・寸法・操作の契約は[キーボード設計](../decisions/0005-snippet-keyboard.md#レイアウトと操作の意味)に定義する。

一覧の挿入と「…」は隣接する別Button。詳細を開くだけでは挿入しない。一覧へ戻る際はフィルター・ページ・スクロール位置を保持する。ピン操作によって対象集合が変わる場合は現存する範囲を表示する。ピン留め0件では「すべて」の「…」から追加できることを案内する。

挿入後は対象行のハイライトと結果文を2秒間表示する。コピーはOSへの書込呼出し後、ピンはDBの確定後に結果文を表示する。挿入の通知は本文を入力先へ渡したことを示し、入力先での保存完了を意味しない。権限不足や失敗は理由を示し、成功の表示を出さない。

**確認条件**：1タップ1挿入、「…」で0挿入、長文末尾、戻る位置、ピンの保存結果、原文、権限別操作、変更・削除拒否、ページ・更新・切替・終了を確認する。狭幅・縦横・ライト／ダーク、OSの文字・太字・コントラスト設定変更時の固定表示、VoiceOverの操作を評価する。実施結果は[検証記録](../keyboard-readability-validation.md)に記載する。

## S10 キーボード利用案内

**目的**：利用者がOS設定でキーボードを追加し、必要な権限を判断できるようにする。

設定内のC48から標準のNavigationLinkで開く。利用目的、追加手順、コピー・ピン留めの権限、更新とページ操作、使えない入力欄の順に見出しと文章を一列で配置する。手順の全文が読める自然な高さとスクロールを使い、OS設定を再現する独自スイッチや、取得できない有効化状態は表示しない。

**確認条件**：nibble未追加の利用者が手順を追える。一覧・全文・挿入にはフルアクセスが不要で、コピー・ピン更新には必要なことと、入力先の制約を理解できる。標準の戻るで設定へ復帰できる。
