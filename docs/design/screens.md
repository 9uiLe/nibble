...
タイトルとフィルターは現在地と集合を保持し、内容は縦にスクロールする。下部の作成はF06のsafe-area領域、一覧・検索の通知はTab Bar accessoryへ置く。[通知の設計](decisions/0004-tab-accessory-notices.md)に従い、検索語・フォーカス・スクロールの状態を保ち、タブ移動・シート表示・背景移行で通知を終了する。末尾の項目をボタンの上まで移動できることを評価する。

検索入力中の上部通知は[G13](audit.md#g13-b--検索入力中の通知配置)の評価候補として扱う。配布済みのTestFlight試作はこの配置を含む。現在の比較用実装は[G14](audit.md#g14-b--一時通知によるタブバーの再配置)に従って一覧・検索の通知を上部へ統一し、正式採用の判断と実行結果を区別する。
