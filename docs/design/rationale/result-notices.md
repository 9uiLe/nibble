...
上部safe areaに結果文・対象名・取り消しを置く。通知はナビゲーション付近に一時的に重なり、元画面のsafe areaを変えない。業務上の並べ替えと通知自体の配置変化を分けて評価する。詳細な値は[LibraryNoticeWindow](../../../app/Nibble/Library/LibraryNoticeWindow.swift)、責務と基準はC18/C19を正本とする。

取り消しは44pt以上、処理中は無効とし、対象名は視覚上1行に省略しても読み上げでは全文を伝える。モーダルな読み上げ領域にはしない。別のSwiftUI rootにも[固定表示](fixed-interface.md)を適用する。独自の出現/消去アニメーションは付けない。
