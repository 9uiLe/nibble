...
| `DeletedSnippetsView` | 削除一覧のモデルとタスク所有者を保持し、シートとsceneの寿命から通知の表示資格を決める |
...
`NoticeContext`は、ある画面が通知を表示できる一回の滞在を識別する値である。一覧・検索では、シーンがactive、設定が閉じている、編集と削除一覧のシートが閉じている場合に表示を許可する。UIからの操作は所有者が受理した時点、操作APIの直接呼出しは処理開始時点のcontextを保存する。
