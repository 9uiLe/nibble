...
| 利用場面 | 設計と正本 |
| --- | --- |
| 項目を探して利用する | 一覧・検索・設定の三領域を保持し、アイコンタブで移動する。スクロール中も全移動先を残す。[C01〜C12](components.md#ナビゲーションと一覧) |
| 入力を始めて終える | 右上の作成操作から共通エディターを開く。検索入力は検索欄のタップで開始する。保存と下書き保持を区別し、操作は読み上げ名を持つアイコンで統一する。[S02・S03・S07](screens.md)、[操作の理由](rationale/library-actions.md) |
