...
| C22 本文入力 | 縦に伸びる標準入力欄で原文を編集する。空または空白のみの場合は本文が保存に必要なことを説明する。原文の空白・改行を保つ説明とUTF-8の長さ上限は入力中も参照できる補足にする | F03の本文スタイルで一覧から編集への文字の変化を減らす。固定高の内側と外側でスクロールが競合する構成を避ける | R14、F09。変換、選択、末尾編集、空白・改行・Unicodeの保持を確認する |
