...
| C02 画面タイトル | 現在地を示す。一覧・検索は固定した左見出しと補足、一覧には保存済み総数を添える。設定は左見出し、階層内とシートは標準inlineタイトルを使う | 内容の前で位置を伝える。一覧・検索は一定の高さで文字の階層を付け、スクロールで見出しを伸縮させない。左見出しには狭幅での識別性が必要 | R03・R05、F03。狭幅・固定した標準文字で識別でき、見出しを重複して読み上げない |
...
| C07 下書き行 | 通常一覧は編集記号・「下書きを再開」・最新1件の表示名を帯で示す。下書き専用一覧ではタイトル・本文要約・更新日時で識別する。無題は本文先頭を名前にする。タップで再開し、コピーは置かない | 保存済みと未完了の内容を区別する。同じ行形式と操作にすると利用対象を取り違えやすい | R04・R13。複数の下書きを識別し、選んだ編集を再開できる |
| C08 追加取得 | List末尾の「さらに表示」で続きの保存済み項目または下書きを取得する | 続きの存在と読込の開始を明示する。無限スクロールより終端と取得の制御を優先する | R12、F07。100件単位で取得し、表示済みの対象とフォーカスを保つ |
| C09 新規作成 | 内容がある一覧・検索の下部に、accentで塗った正円の＋を置き、左右設定を反映する。作成可能な空一覧では文言付きの作成入口に集約する | 項目内の操作から独立させ、片手で届く範囲を確保する。全タブ共通の操作にすると設定にも現れる | R07・R18、F05・F06。最終行を隠さず、検索入力中は表示しない |
| C10 コピー | 対象行の横に淡い角丸の背景と44ptの領域を持つコピーボタンを置く。本文をコピーする。OSへの書込呼出しが戻った時点を使用として記録し、順位を再取得する | 対象を開かずに利用できる。詳細画面への移動が必要な構成より操作を減らす | R07・R11、F05。行の編集と誤って同時実行せず、対象名と操作を読み上げる |
