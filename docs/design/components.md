...
| --- | --- | --- | --- |
| C01 標準タブ | iOS標準TabViewのLiquid Glassで一覧・設定・検索を同じタブ列にまとめ、TabIconの20ptテンプレート画像のみを表示する。読み上げ名と領域ごとの状態は保持する。下方向へのスクロールで標準タブバーを縮小し、上方向へのスクロールまたは縮小したバーの操作で再展開する | 内容を読む面積を増やし、検索も同じ階層の移動先として扱う。画像の表示寸法とOSの操作領域を分け、素材・選択・縮小の動作はネイティブに任せる | R01・R21、F05・F06。縮小後も各タブへ到達でき、検索と一覧の状態が独立し、ライト／ダークで記号を識別できる |
...
| C09 新規作成 | 内容がある一覧・検索の右下に、accentで塗った正円の＋を置く。作成可能な空一覧では文言付きの作成入口に集約する | 項目内の操作から独立させ、片手で届く範囲を確保する。全タブ共通の操作にすると設定にも現れる | R07・R18、F05・F06。最終行を隠さず、検索入力中は表示しない |
...
| ID・部品 | 責務と構成 | 配置理由と制約 | 根拠・確認条件 |
| --- | --- | --- | --- |
| C12 標準検索欄 | 検索タブを選ぶと下部ツールバーの標準検索欄へ入力を開始し、語句の消去・確定・検索終了を行う。タブの役割と検索欄の配置を独立させる | 覚えている言葉をすぐ入力できる。見出しを他のルート画面と揃え、片手で届く位置に入力を置く | R02・R19。タイトル・本文が対象と分かり、日本語変換と検索確定を区別できる |
