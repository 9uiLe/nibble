...
| --- | --- | --- | --- |
| C01 フローティングタブ | 一覧・検索・新規作成・設定を、アイコンのみの同じカプセル形バーに置く。通常60pt、縮小48ptの高さで、四つの入口を残したまま幅と記号を縮める。下スクロールで縮小、上スクロール・画面切替で復元する。読み上げ名・選択状態と領域の状態を保持する | R21の利用者提示画像に合わせる。素材は標準glassEffect、寸法・選択表示・動きはアプリが所有する。内容の表示領域は固定し、跳ね返り・読込・通知で切り替えない | R01・R21、F05・F06。最小44ptを保ち、縮小中も四つの入口を直接操作できる。ライト／ダークと狭幅で識別できる |
| C02 画面タイトル | 一覧・検索・設定に共通のScreenHeadingを使う。titleと任意のsubTitleを受け取り、F03の画面タイトルを左揃えで固定する。設定は補足なし、一覧は保存済み総数、検索は対象案内を添える。階層内とシートは標準inlineタイトルを基本とし、編集はC24・C25と並ぶ固定ヘッダーに置く | 同じ階層の現在地を同じ文字の強さで伝える。補足の有無だけを入力で表し、文字・余白・読み上げ属性を部品に集約する | R03・R05、F03。狭幅でも識別でき、補足なしの空行や見出しの重複読み上げがない |
...
| C09 新規作成 | C01の三番目の＋で編集シートを開く。現在の領域とフィルター・検索語を保持し、終了時は呼出元へ戻る。空一覧の文言付き作成入口も保つ | 利用者の指定により共通バーへ集約し、独立した浮遊ボタンを除く。移動する三領域と異なり、作成は選択状態を切り替えない操作ボタンとする | R07・R18、F05・F06。一覧・検索・設定から作成でき、重複提示しない。検索入力中はバーごと隠す |
...
| C22 本文入力 | 縦に伸びる標準入力欄で原文を編集する。空または空白のみの場合は本文が保存に必要なことを説明し、空の新規作成では本文にフォーカスする。原文の空白・改行を保つ説明は入力欄に添え、UTF-8の長さ上限はC31の補足シートへ置く | F03の本文スタイルで一覧から編集への文字の変化を減らす。固定高の内側と外側でスクロールが競合する構成を避ける | R14、F09。変換、選択、末尾編集、空白・改行・Unicodeの保持を確認する |
| C23 ペースト | 本文ラベル横のPasteButtonと「末尾に追加」でテキストを末尾へ追記する | 本文へ加える操作をその対象の近くに置き、明示的な入力を入口にする | R13、F09。選択位置への挿入と追記を取り違えないか評価する |
| C24 保存 | 上部バーの末尾側に文字の「保存」を強調し、保存済み項目への反映を確定する | 標準の完了位置と明示的な操作名を使う。自動保持は下書き、保存は利用対象への確定という意味を保つ | R05・R09。本文が空白のみなら無効。成功時だけ閉じ、重複保存しない |
| C25 閉じる | 上部バーの先頭側に文字の「閉じる」を置き、下書きを保持して戻る。下部の補足でこの終了方法を案内する | 中断と確定保存を分ける。入力を捨てる意味に受け取られないよう補足する | R09・R13。最新入力の保持成功を待ち、失敗時は画面に残す |
...
| C31 終了の補足 | キーボードを閉じた画面下部に「保存」「閉じる」の結果を短く示す。「入力の上限と保存について」からスクロール可能な補足シートを開き、UTF-8上限と空入力・未変更時に下書きが残らない例外を説明する。キーボード上にも入口を置き、戻ると入力先を復元する | 日常の入力を圧迫せず、判断に必要な条件を入力中も読めるようにする | R13、F09。主要操作名との意味が一致し、補足の往復でも入力内容と入力先を保つ |
