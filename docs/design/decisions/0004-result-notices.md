...
状態：採用。判断日：2026-09-19。対象：S01・S02、C18・C19、F06〜F08。関連課題：[Issue #26](https://github.com/9uiLe/nibble/issues/26)。
...
| 要素 | 仕様 | 理由 |
| --- | --- | --- |
| 表示位置 | 通知ウィンドウの上端safe areaから8 pt、左右16 pt、中央揃え、最大幅520 pt | 検索欄・キーボード・下部操作と位置を分け、小画面でも結果と取り消しを収める |
| 通知カード | 最小高さ52 pt、角丸20 pt、標準の`regularMaterial` | 内容の上に重なる短時間の結果を、背景と区別して示す |
| 結果文と対象名 | 結果文を優先し、対象名はcaption・1行省略 | 完了と回復操作を常に読める幅を確保する |
| 取り消し | 文言「元に戻す」、最小高さ44 pt、復元中は無効 | 操作の意味と進行中の重複防止を明確にする |
| 出現と消去 | 独自アニメーションなし | 一時通知の変化を元画面の配置やアニメーションへ伝播させない |
| 配色と文字 | シーンのライト・ダーク、`NibbleInterface`による固定表示 | 本体と同じ文字サイズ・太字・コントラストの契約を別のSwiftUI rootにも適用する |

通知はナビゲーションタイトル付近に一時的に重なる。元画面のsafe areaへ領域を追加しないため、通知の表示前・中・消去後で一覧、フィルター、タブ、作成ボタンの配置は変わらない。コピーに伴う使用回数順の変更など、業務操作による内容更新は別に扱う。

対象名の全文はアクセシビリティラベルと取り消しボタンのヒントへ含める。通知をモーダルなアクセシビリティ領域にはせず、元画面の要素を利用できる状態にする。文字設定と演出の共通契約は[固定表示方針](0002-fixed-interface.md)に従う。
...
最低対応OSはiOS 26.0、実行検証はiOS 26.5のみとする。コンパイル、AX、画像、抽出フレーム、動画の全編再生は保証する範囲が異なる。VoiceOver音声、触覚の体感、実機性能をSimulatorの結果から推定しない。対象ソース・観測・失敗・未実施条件は[通知の検証記録](../../notice-validation.md)に記録する。
