...
[全290定義の監査台帳](test-audit-inventory.csv)は基点時点のメソッド名・位置と、残す保証・候補IDを記録する。以下は静的監査の54候補と実行で見つかったU06・U07の計56候補で、部分削除や条件付き候補も含み、削除したメソッドの件数ではない。対象リンクは削除前のコードを特定する。欠番S04・P18は精査で棄却した候補であり、下記の「削減しない保証」に理由を記す。
...
### U06 コンパクト端末に合わない座標・入力前提

対象：[MVP UI driver](../scripts/check-mvp-ui.py)の`check_navigation_title`と本文ペースト。

実施：`navigation.title`で製品の見出しを特定し、画面上部・左寄せ・表示幅の期待を保持したうえで、Y座標50以上という下限を外した。

判断：iPhone SE（第3世代）、iOS 26.5で正しい見出しがY=30となり、旧assertionが失敗した。safe areaの異なる端末を同じ座標下限で扱う追加価値はなく、製品の誤配置とは区別する。失うのは正常な上余白を特定の端末形状へ固定する保証だけで、見出しの欠落・右寄せ・下方配置は引き続き失敗する。本文の貼付けでは、タイトル入力後のキーボードが本文TextFieldのAX矩形中心を覆っていた。入力欄をフォーカスしてキーボード表示を待ち、AX矩形とキーボード上端から求めた可視領域へジェスチャーを送る。フォーカスに伴うメニュー位置の変化と、隠れた矩形中心への操作を避ける。短い検証用テキストの末尾側余白を操作し、単語途中の部分選択でメニューが出ない状態を避ける。貼付け後のUTF-8完全一致は維持する。操作コマンドの成功だけで次の操作へ進まず、AXの入力反映を待つ。AXが空白を省略するため、この待機と保存・コピー後のbyte検査は分ける。狭幅の日本語編集メニューでは「すべてを選択」が「進む」の先にあるため、観測したメニューを展開する。既知のSelect All取得失敗だけを扱い、編集後コピーの完全一致と対応付ける。

### U07 遷移途中の空UIを即時失敗にする待機

対象：[MVP UI driver](../scripts/check-mvp-ui.py)の`wait_ui`、[About driver](../scripts/check-about-ui.py)の設定スイッチ待機と[共通driver](../scripts/ios.py)の`Run.ui`。

実施：状態への到達を待つときだけ空のUI取得を許容し、既存の有限回pollを続ける。通常の単発読取、ツールの失敗、対象processの終了は引き続き失敗する。空の取得結果を条件成立とは扱わず、各取得結果はrunへ保存する。これらの違いを1つの回帰テストで確認する。

判断：編集シートを閉じた直後にsim-useが成功・要素0件を返し、`wait_ui`が期待状態の到達を待つ前に失敗した。遷移途中の空snapshot自体は製品の不具合ではない。最終状態を検査する保証を残し、描画途中の一時状態への依存を外す。待機の上限と最終状態のpredicateは緩めない。Aboutでもアプリ切替直後に設定スイッチが見えず失敗したため、SettingsのappPackage・スイッチ・値がそろうまで有限回で待つ。設定変更後も要求値を観測してから進む。
...
変更後の製品テストは101定義成功、同じ端末・Releaseで5.296秒。通知期限の統合先は0.007秒、表示入力を追加したmounted rowは2.581秒だった（変更前1.833秒）。Pythonの最終実行はU07の回帰検査を含む147メソッド成功・skipなし、2.077秒。各1回の比較なので通常の揺らぎを含み、恒常的な高速化率や不安定さの改善率を保証する値ではない。検証hostは2定義から1定義へ統合し、原文反映まで成功した。
...
同じmacOS 26.2 arm64、Xcode 26.5（17F42）/ Swift 6.3.2、iPhone SE（第3世代）の専用Simulator `A4B7ED78-6662-4055-9528-003497F1F0A1`、iOS 26.5（23F77）を使用した。コマンドはリポジトリルートのNix lock環境で`NIBBLE_UI_FORMAT=json`を指定した。基点の製品runは`20260919T163232Z-test-ec5a2a`で、基点コミットとの入力照合が成功している。

途中の失敗は`artifacts/ios/`の別runとして保持する。Swiftでは統合helperの引数名と、同じsequenceの`updateDraft`をthrowと誤認した期待を修正した。MVPではU06・U07の座標、貼付け反映、狭幅メニュー、遷移待機を修正した。Aboutは開始条件であるSettingsのMotion画面を準備し、切替直後の読取をU07で修正した。失敗試行で残ったダミー下書きが一覧を占め、対象行が画面外になる試行もあった。最終MVPは、この作業で作成した専用Simulatorのnibbleだけをアンインストールして空のデータから実行する。既存のユーザー端末・データは対象にしない。設定復元を含む最終runの結果で成否を判断し、失敗runを成功へ書き換えない。

### 最終実行とソース照合

実行入力を確定したコミットは`099bcf5a73e7fb0517de9dad303c8f3739dc566b`。各runは`artifacts/ios/<run ID>/`へ保存した。後続の文書だけのコミットで再利用する場合も、対象revisionへの入力照合を行う。

| 入口 | run ID | 結果 |
| --- | --- | --- |
| `ios.py test --project-config app/project.json --configuration Release --device <専用UDID>` | `20260919T175955Z-test-ec42ff` | 101定義（parameter展開後116）成功、失敗・skipなし、テスト本体5.296秒 |
| `ios.py test --project-config validation/project.json --configuration Release --device <専用UDID>` | `20260919T180820Z-test-c6d194` | 1定義成功、失敗・skipなし |
| `check-mvp-ui.py --device <専用UDID>` | `20260919T175252Z-mvp-ui-f18f3f` | 成功。新規・下書き復帰・編集・UTF-8完全一致のコピー・pin・操作位置切替・削除取り消し・削除した項目の検索を確認 |
| `check-about-ui.py --device <専用UDID> --reduce-motion enabled` | `20260919T175657Z-rive-about-reduced-e7011a` | 成功。起動・継続再生・背景復帰・動作設定切替・light/dark/最大文字で末尾到達、元設定へ復元 |
| `check-interface-ui.py --device <専用UDID>` | `20260919T175904Z-fixed-interface-9b86c4` | 成功。通常と最大文字・高コントラストの2条件で一覧・編集・設定・Aboutの対象frameが一致、元設定へ復元 |

全5runで`check_evidence.py --integrity-only --ref 099bcf5a73e7fb0517de9dad303c8f3739dc566b`が成功し、開始・終了・コミットの入力、成否、媒体hashが一致した。共通の`nix flake check --no-update-lock-file --print-build-logs`は全7検査成功。Python147件の計測結果は`artifacts/test-consolidation/python-timings.json`、単独計測6回は`artifacts/test-consolidation/store-benchmark/results.json`へ保存した。

原本PNGと録画の各3抽出時点を確認し、それぞれの`review.json`へ方法・実時刻・観測・制約を記録した。MVPは192.488秒の録画から9.506667秒・95.661667秒・173.408333秒、Aboutは82.373秒から4.13秒・41.046667秒・74.406667秒、固定表示は28.115秒から0.945秒・14.02秒・25.323333秒を確認した。

MVPの原本で編集後コピーの通知と対象名付き取り消し通知、固定表示で同じ文字サイズと配置、Aboutの`loop-3`→`loop-5`→`loop-6`で保存完了→再び複製が移動→次の保存完了を観測した。Aboutの4.13秒の抽出画像ではcaptionはあるが図は未表示で、後続の原本PNGには図が表示される。読み込み時間や全フレームの連続性は測定していない。

全編再生、VoiceOver音声操作、実機、配布は今回の確認範囲に含めない。変更していないResearchProbeの14定義は精査したが再実行対象にはしていない。製品UIの変更ではないため、媒体公開・ブラウザー閲覧と公開用の完全な証跡検査は対象外とし、公開・閲覧欄は空のまま残す。`--integrity-only`の成功を媒体の公開や全編確認の完了と扱わない。

必要な入口は[製品手順](mvp.md)と[検証fixture手順](ios-verification.md)、PRの照合条件は[証跡とPRの検査](review-evidence.md)に従う。
