...
1. 新規作成で本文に `前 後` を入力し、両語の間へカーソルを置く。「変数を追加」で `宛名` を作り、本文が `前 {{宛名}}後` のように**選んだ位置**へ変わることを確認する。印の直後で文字を入力し、カーソルもその位置にあることを確認する。文字を選択した状態では、その範囲だけが印に置き換わることを確認する。
   追加シートでは既存名がない案内、名前が空・重複・長すぎる・波括弧か改行を含む場合の理由とボタン状態、名前欄を選んだキーボード表示を確認する。「追加せず閉じる」は追加シートだけを閉じ、編集画面の入力を残すことを確認する。既存名がある場合は追加できる行として見えることと、選択すると同じ印が本文に増えることを確認する。通常の高さと全画面で行・名前欄・確定操作が隠れないか確認する。
