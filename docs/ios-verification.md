...
2. フルアクセスなしで行の「入力」を含む主領域をタップし、1回の挿入と原文のUTF-8を照合する。「全文を見る」は挿入せず詳細へ進み、「すべて」「ピン留め」の双方で戻った後の集合・ページ・スクロール位置と長文末尾を確認する。同じ変数を複数箇所で使う本文は値の入力欄が一つだけで、完成文の全箇所が置き換わることを確認する。
