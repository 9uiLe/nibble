...
| 操作位置と読みやすさを保つ | 一覧・設定・検索はネイティブのLiquid Glassタブで切り替える。作成は右下の円形＋、行操作は右側に固定する。本文の文字組を一覧・編集・説明で統一する。一覧・検索・設定は共通見出しと任意の補足を使い、タブの記号は小さいテンプレート画像とする | [共通原則](design/foundations.md)、[部品](design/components.md) |
