...
無料プランは削除されていない保存済み項目を30件まで作成できる。下書きと削除済み項目は数えない。Proの月額・年額自動更新サブスクリプションは作成数の上限を解除する。価格と更新期間はStoreKitが取得した商品情報を表示し、アプリ内へ固定価格を埋め込まない。サブスクリプションの権利はApple Accountで復元できるが、保存した本文は端末間で同期しない。

作成数は新しい保存済み項目を確定するtransaction内で確認する。本体・共有拡張・競合からの「新しい項目として保存」に同じ規則を適用する。既存項目の編集、下書き保持、検索、コピー、キーボード挿入、削除済み項目の復元は上限で妨げない。上限で新規保存に失敗しても入力と下書きを保持する。Proの期限切れや返金後に30件を超えていても、既存項目の利用は続けられ、新規保存だけを制限する。

本体はStoreKitが検証した現在の権利と取引更新からPro状態を再評価する。共有拡張はApp Groupに保存した期限付きの権利情報を読み、同じ保存規則を適用する。購入と復元の操作は本体だけで行う。無料枠の到達や画面操作を外部analyticsへ送らず、利用・購入・継続の集計にはApp Store Connect Analyticsだけを用いる。広告はこの段階では表示しない。

App Store Connectでは同じサブスクリプショングループに月額`nibble.pro.subscription.monthly`と年額`nibble.pro.subscription.yearly`を作る。商品名・説明・販売価格・販売地域・規約とプライバシーポリシーのリンクをASCで設定する。日本での初期価格は月額300円・年額2,400円とし、公開前に販売条件と継続的な価値を確認する。商品がASCに存在しない環境ではStoreKitの商品選択・購入・復元を完了できないため、保存上限の自動テストと購入のSandbox検証を別に記録する。
