...
本体は縦向き固定とし、端末の回転で画面を横向きへ切り替えない。共有拡張とキーボード拡張の表示環境は呼出元アプリが所有し、本体の画面方向設定では変更しない。
...
| 項目を整理し、削除から回復する | ピン留め・解除、削除、通知からの取り消しを提供する。設定から階層移動する削除一覧では、行の復元と完全削除を別のボタンで選ぶ。完全削除は対象名と取消を示す確認アラートを経て、関連下書きも消す | [操作完了](#非同期操作の完了契約)、[通知](design/rationale/result-notices.md) |
| 他アプリとの間で利用する | 共有されたテキスト・URLを編集して保存できる。Keyboardは「すべて」を初期表示してピンを先頭に置き、行タップ1回で本文を1回挿入する。右端から全文を開き、本文の末尾まで読んで挿入またはコピーを選べる。詳細から戻ってもフィルターと一覧位置を保つ | [共有](#呼び出しと共有の契約)、[Keyboard仕様](architecture/keyboard.md) |
| 操作位置と読みやすさを保つ | 一つの作業画面に検索と集合選択をまとめ、設定は階層で開く。作成は画面下部の文字ボタン、行のコピーと整理は右側へ置く。検索は欄をタップして入力を始め、検索語を消すと元の集合と取得上限へ戻る。編集の保存と閉じるは文字で区別し、本文を一つのスクロール領域で編集する | [共通原則](design/foundations.md)、[部品](design/components.md) |
| 使い方とデータの扱いを伝える | 設定からキーボードの追加・権限とアプリ情報を読める。アプリ情報から公開プライバシーポリシーを開ける。説明イラストを操作完了の証拠にしない | [画面とフィードバック](#画面とフィードバック) |
...
保存・検索・コピー・キーボード挿入は件数の上限なく利用できる。機能ごとの無料/Pro判定は`FeatureAccess`に集約し、本体と拡張で同じ判断を使う。差し替え機能も無料で使える。Proだけに提供する機能を設定する場合は、この判定と購入案内を合わせて更新し、保存した原文を失わせない。Pro向けの継続的な価値が確定するまで購入画面は表示しない。実装上の責務と設定箇所は[収益と機能アクセス](architecture/monetization.md)に定める。

本文に`{{宛名}}`のような印を入れると、コピーまたはキーボード挿入の直前に値を入力する。編集画面では既存の変数を再利用するか、新しい変数を作成するかを選び、印を本文のカーソル位置へ挿入する。文字を選択していた場合はその範囲を印で置き換え、本文に選択位置がない場合は末尾へ追加する。選択位置が無効なら本文を変えず、位置を選び直せるようにする。同名の印は一度の入力で全箇所を置き換える。名前は前後の空白を取り除いた後に空でない40文字以内とし、波括弧と改行を含めない。値がそろうまで実行できず、確認前の取消ではコピー・挿入・使用記録を行わない。差し替えは利用時の出力にだけ適用し、保存した本文を変更しない。変数のない本文は値入力を挟まず利用する。キーボード挿入では確認中に入力先が変わった場合、元の入力先へ送らない。利用不可の状態でも変数を含む原文を保存・編集できるが、コピー・挿入では利用不可の理由を示して実行しない。

本体はStoreKitが検証した現在の権利と取引更新からPro状態を再評価し、App Groupに期限付きの権利情報を保存する。購入と復元の操作は本体だけで行う。サブスクリプションの権利はApple Accountで復元できるが、保存した本文は端末間で同期しない。商品を表示する段階では価格と更新期間をStoreKitから取得し、固定価格を埋め込まない。

無料版への広告表示を予定する。広告の配信先・識別子、プライバシーポリシー、同意表示を整えるまで広告SDKと広告枠は組み込まない。広告を導入してもキーボード・編集・差し替え・コピー完了の画面には表示しない。画面操作や本文を外部analyticsへ送らず、購入・継続の集計にはApp Store Connect Analyticsを用いる。

App Store Connectには同じサブスクリプショングループで月額`nibble.pro.subscription.monthly`と年額`nibble.pro.subscription.yearly`を登録済みである。日本での設定価格は月額300円・年額2,400円。商品名・説明と販売条件はPro公開時に提供内容へ合わせて見直し、規約とプライバシーポリシーのリンクを設定する。商品がASCに存在しない環境ではStoreKitの商品選択・購入・復元を完了できないため、機能の自動テストと購入のSandbox検証を別に記録する。
...
| collection / query | 閲覧する集合 / 保存済み全体から探す検索語。検索しても元の集合を失わない |
| request / snapshot | 選択した検索・フィルター・取得範囲 / 同じ要求とDB読取時点の結果を固定した組 |

### 原文と検索用の値

共通エディターはMarkdownの見出し・太字・斜体・コード・引用・リンク・取り消し線を入力中の文字へ反映する。装飾は表示だけに適用し、Markdown記号、空白、改行を入力・保存・コピーから取り除かない。未確定のIME入力には装飾を適用せず、確定後に更新する。「プレビュー」は記号を隠して見出し・強調・段落・リストなどを読む表示とし、「入力」へ戻ると原文の編集を続けられる。開いたときは「入力」を選び、切替で原文・選択・取り消し履歴を失わず、保存・コピーには常に原文を使う。表示モードは永続化しない。原文とプレビューは一つの解析結果と文字スタイルを共有し、見出し・強調の意味を揃える。解析結果が現在の原文に追い付いていない間は進行を示す。
...
入口が依存を組み立ててinitializerで渡す。UIからグローバルな保存層を探索しない。本体の閲覧と検索は一つのLibraryModelが要求・結果・編集の提示を所有し、削除一覧は同じ保存層を使う独立したモデルを持つ。共有拡張は別プロセスの保存層を同じ編集機能へ渡す。
...
一覧・検索を統合したLibraryModelはシーンのルートAppRootView、画面操作はLibraryTaskOwner、編集入力とフォーカスはSnippetEditorが所有する。編集シートもルートから提示し、設定への移動や背景移行で提示元を失わない。
...
| 検索・削除一覧の検索語を変更して取得中 | 表示中snapshotを操作不可で保持し、要求した検索の読込中であることを示す |
| 本体一覧の保持済みフィルターを選択 | 同じDB時点の対象ページを即座に表示し、読込を開始しない |
| 同じ集合の更新・追加取得 | 表示中の内容を保持する |
| 成功 | 要求と結果を一緒に反映し、一覧または0件案内を表示 |
| 失敗 | snapshotを保ち、要求に対応する再試行を表示 |
| 中断 | 進行表示を終え、同じ要求の「読み込みを再開」を表示 |

本体は初回にすべて・ピン留め・下書きの各ページを同じ読取transactionで取得し、LibraryReadStateへ保持する。選択集合・検索語・集合ごとの取得上限を入力状態の正本とし、LibraryRequestはそこから導出する。検索は保存済み全体を対象にする独立した読取結果であり、閲覧用の3集合を上書きしない。検索語を消すと、保持した集合と取得上限へ即座に戻る。検索語だけの変更は検索ページだけを取得する。Pull to Refresh、編集終了、コピー・ピン・削除・復元などの操作後は3集合と必要なら検索結果を同じ読取transactionで更新する。フィルター切替自体や検索していない状態のactive復帰では再取得しない。検索中と削除一覧は表示・active復帰で再取得する。

URLから一覧を開く要求と、ピン留めの空状態から「すべてを見る」操作は、Applicationの`showAll()`で選択を「すべて」にし、検索語を消す。保持済みの「すべて」の取得上限は維持する。削除一覧ではこの要求を受け付けない。
...
画面内の作成操作は、呼出元の画面・選択集合・検索語を変更しない。終了後も同じ対象から利用を続けられるようにする。保存・下書き保持の結果は各集合の条件に従って反映する。URLで一覧を指定する呼び出しは[呼び出しの契約](#呼び出しと共有の契約)に従う。
...
本文のMarkdown解析は、本文のUTF-8が変わったときだけ開始する。入力の連続更新を80 msまとめてからUI actorの外で解析し、完了時に現在の本文と一致する結果だけを表示する。解析用の`bodyRevision`は本文の表示更新、永続化用の`sequence`はタイトルと本文の入力更新を識別する。同じ解析結果から入力装飾とプレビューのブロックを作り、表示切替だけでは再解析しない。解析待ちのプレビューには準備中を示し、古い本文を表示しない。解析結果から原文を書き戻さない。

装飾範囲は解析用テキストのUTF-8列から原文のUTF-16位置へ対応付ける。LF・CR・CRLFの改行と、Markdown解析で置換文字として扱うNULを考慮する。原文の改行・NUL・結合文字・絵文字を変換せず、その文字位置へ属性だけを適用する。

説明の入口は「入力と保存について」と表示する。末尾へのペーストはOS標準のPasteButtonを使い、テキスト形式のclipboardがあるときに利用できる。ペースト可能な内容をアプリが先読みせず、利用者の操作で受け取った原文を本文の末尾に追加する。保存・保持・破棄の処理中は入力と追加操作を受け付けない。

編集は一つの外側ScrollViewで入力全体をスクロールする。フォーカス中は不透明な背景を持つキーボード上部の操作、入力終了時は画面下部の操作を同じsafeAreaInsetへ排他的に配置する。OSのキーボードsafe areaと合わせて入力領域を確保する。補足シートの表示中はフォーカスを退避し、閉じたら元の入力欄へ戻す。
...
`nibble://library`は検索語と選択集合を初期化して作業画面へ戻り、`nibble://new`は同じ初期化後に新規下書きを開く。この2種類だけを受け付け、path・query・fragment・認証情報・port付きURLを拒否する。custom schemeは所有権を保証しないため、本文や保存・削除指示をURLへ載せない。
