...
本文に`{{宛名}}`のような印を入れると、コピーまたはキーボード挿入の直前に値を入力する。同名の印は一度の入力で全箇所を置き換える。名前は空でない40文字以内とし、波括弧と改行を含めない。値がそろうまで実行できず、確認前の取消ではコピー・挿入・使用記録を行わない。差し替えは利用時の出力にだけ適用し、保存した本文を変更しない。変数のない本文は確認を挟まず従来どおり利用する。キーボード挿入では確認中に入力先が変わった場合、元の入力先へ送らない。
