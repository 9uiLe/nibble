...
保存・検索・コピー・キーボード挿入は件数の上限なく利用できる。機能ごとの無料/Pro判定は`FeatureAccess`に集約し、本体と拡張で同じ判断を使う。現行では差し替え機能も両方で使える。将来のPro限定化はこの判定と購入案内を合わせて変更し、保存した原文を失わせない。Pro向けの継続的な価値が確定するまで購入画面は表示しない。

本文に`{{宛名}}`のような印を入れると、コピーまたはキーボード挿入の直前に値を入力する。編集画面では既存の変数を再利用するか、新しい変数を作成するかを選び、印を本文末尾へ追加する。同名の印は一度の入力で全箇所を置き換える。名前は空でない40文字以内とし、波括弧と改行を含めない。値がそろうまで実行できず、確認前の取消ではコピー・挿入・使用記録を行わない。差し替えは利用時の出力にだけ適用し、保存した本文を変更しない。変数のない本文は確認を挟まず従来どおり利用する。キーボード挿入では確認中に入力先が変わった場合、元の入力先へ送らない。利用不可の状態でも変数を含む原文を保存・編集できるが、コピー・挿入では利用不可の理由を示して実行しない。
