...
本文のMarkdown解析は、本文のUTF-8が変わったときだけ開始する。入力の連続更新を80 msまとめてからUI actorの外で解析し、完了時に現在の本文と一致する結果だけを表示する。解析用の`bodyRevision`は本文の表示更新、永続化用の`sequence`はタイトルと本文の入力更新を識別する。解析結果から原文を書き戻さない。
