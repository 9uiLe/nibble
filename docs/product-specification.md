...
本体一覧は初回にすべて・ピン留め・下書きの各ページを同じ読取transactionで取得し、シーンのモデルに保持する。フィルターとメインタブの切替やactive復帰だけでは再取得せず、保持済みの集合を直ちに表示する。外部変更はPull to Refreshで3集合へ取り込む。保存・コピー・ピン・削除・復元などアプリ内の操作後は3集合を更新する。各フィルターの取得上限は切替をまたいで保持し、未取得分を手元の「すべて」から抽出してピン留めを欠落させない。検索・削除一覧は検索語の変更・表示・active復帰で独立した要求を再取得する。本体一覧に検索語や削除フィルターを持たせず、検索画面には保存済み全体、削除一覧には削除済み項目を固定する。
...
画面内の作成操作は、呼出元のタブ・一覧フィルター・検索語を変更しない。終了後も同じ集合から利用を続けられるようにする。保存・下書き保持の結果は各集合の条件に従って反映する。URLで一覧を指定する呼び出しは[呼び出しの契約](#呼び出しと共有の契約)に従う。
...
`nibble://library`は検索画面の検索語と一覧のフィルターを初期化して一覧を選択し、`nibble://new`は新規下書きを開く。この2種類だけを受け付け、path・query・fragment・認証情報・port付きURLを拒否する。custom schemeは所有権を保証しないため、本文や保存・削除指示をURLへ載せない。
