...
App Store Connectでは同じサブスクリプショングループに月額`nibble.pro.monthly`と年額`nibble.pro.yearly`を作る。商品名・説明・販売価格・販売地域・規約とプライバシーポリシーのリンクをASCで設定する。価格は月額300円・年額2,400円を検証の初期仮説とし、公開前に販売条件と継続的な価値を確認する。商品がASCに存在しない環境ではStoreKitの商品選択・購入・復元を完了できないため、保存上限の自動テストと購入のSandbox検証を別に記録する。
