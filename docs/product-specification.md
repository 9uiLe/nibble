...
本文に`{{宛名}}`のような印を入れると、コピーまたはキーボード挿入の直前に値を入力する。編集画面では既存の変数を再利用するか、新しい変数を作成するかを選び、印を本文のカーソル位置へ挿入する。文字を選択していた場合はその範囲を印で置き換え、本文に選択位置がない場合は末尾へ追加する。同名の印は一度の入力で全箇所を置き換える。名前は空でない40文字以内とし、波括弧と改行を含めない。値がそろうまで実行できず、確認前の取消ではコピー・挿入・使用記録を行わない。差し替えは利用時の出力にだけ適用し、保存した本文を変更しない。変数のない本文は値入力を挟まず利用する。キーボード挿入では確認中に入力先が変わった場合、元の入力先へ送らない。利用不可の状態でも変数を含む原文を保存・編集できるが、コピー・挿入では利用不可の理由を示して実行しない。
