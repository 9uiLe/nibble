...
| 表示値の比較 | 全Viewへ`@Equatable`を付与。値だけの部品は`@MainActor EquatableBodyView`で全入力を比較し、通常のViewの親入力はprivateな`inputRevision`で差し替えを反映 | 入力の変更・復元、別Bindingへの差し替え、同じ入力での外観・文字サイズの更新を検査 |
...
生のTask・別scheduler・直接アニメーション・直接比較ゲート・手書き`==`をLintで禁止する。比較除外はライブラリ規約のrevision付きViewの不変入力だけに限定する。構造化されたasync/await・task group・SwiftUI `.task`・協調用Task API、通常のView、値型・enumの標準Equatable合成は許可する。
