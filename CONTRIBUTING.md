...
| hamio 0.1.0（macOS arm64 / Linux） | スクリプトの進捗・結果表示。対応環境と代替出力は[スクリプトの設計](docs/script-tooling.md)を参照 |
...
[開発スクリプトの設計](docs/script-tooling.md)に従い、各CLIが処理順序、成否、実行記録を管理する。共通の接続層である`scripts/script_ui.py`は、公開可能な工程名と結果をhamioで整形する。進捗・診断は標準エラー出力（stderr）、結果JSONやPRコミット表は標準出力（stdout）へ出す。AI・CIは`NIBBLE_UI_FORMAT=json`を指定し、業務の成否をスクリプトの終了コードと結果データで判断する。

hamioによる表示に失敗した場合は警告と代替JSONをstderrへ出し、業務処理の状態を保つ。配布の認証設定・引数・生ログ・例外全体は表示へ渡さない。配布スクリプトは`python3 -I`で起動し、自身に隣接する表示用モジュールを読み込む。
...
Viewを実装するときは、値だけで表示が決まる部品、親から接続先を受け取る部品、StateやEnvironmentだけを持つ画面のどれに当たるかを判断する。[比較規約](docs/library-policy.md#viewの比較境界)が宣言方法、状態の寿命、比較除外の条件を定める。比較用UUIDを表示のIDに使わず、入力更新で編集内容や再生状態が失われない構成にする。

生のTask・別scheduler・直接アニメーション・直接比較ゲート・手書き`==`をLintで禁止する。`@SkipEquatable`は、同じViewで`inputRevision`を比較する不変`let`の親入力に限る。構造化されたasync/await・task group・SwiftUI `.task`・協調用Task API、値型・enumの標準Equatable合成は許可する。

Lintは構文を検査する。型解決・マクロ展開・外部APIの副作用・操作の完了はcompiler、テスト、レビューで確認する。抑制コメントを設けず、許可する入口や構文の変更は規則と回帰テストに反映する。

説明イラストのホストはSessionを保持し、説明文、配色、動作設定、可視性、読込失敗時の回復を管理する。演出はコピーや保存の業務状態と分ける。制作とiOSの接続は[演出設計](docs/decisions/0004-rive-presentation.md)、他製品での利用は[RivePresentationの契約](app/Packages/RivePresentation/README.md)を参照する。
...
| 表示設定と意味情報 | 文字サイズ・太字・コントラストを変えても固定した表示を維持すること、Reduce Motionでも独自演出が続くこと。VoiceOverの順序と操作領域は別に確認 |
