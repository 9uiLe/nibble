...
目的と影響範囲を定め、`git status`と差分を確認してから、下表の対象資料を読む。製品の変更は製品仕様、UIの変更は対象の画面・部品・理由、検証CLIの変更は実行と出力の契約を基準にする。資料の内容や作業範囲が変わったときに再確認する。

編集後は[検証計画](docs/ios-verification.md#変更から検証を実行する)で選ばれた工程と準備事項を確認し、検証中のソースを固定して実行する。[状態の要約](docs/ios-verification.md#検証状態を確認する)から失敗・未開始・ログ・runをたどり、追加差分は成功結果から再計画する。手動確認と[証跡・PRの照合](docs/review-evidence.md)までが検証の範囲である。
...
| 変数の印・入力・完成文の契約 | [変数の編集と利用](docs/architecture/variables.md) |
...
同じ事実・ルール・状態の正本を増やさない。[概念ごとの所有者](docs/architecture/README.md#概念ごとの正本と更新経路)を確認し、Domainのルール、Applicationのユースケース、Persistenceの変換、表示の文言と構成のどこへ変更を置くかを決める。新しい経路も同じルールを呼び、Viewの無効化だけで保存・操作の制約を保証しない。テストはその変更で見逃し得る不具合へ対応させ、統合できるケースを[選択基準](docs/testing.md#残す統合する削除する基準)で整理する。
